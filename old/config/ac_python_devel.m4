...
AC_DEFUN([AC_PYTHON_DEVEL],[
	...
	ac_supports_python_ver=`$PYTHON -c "import sys, string; version = string.split[(sys.version)][[0]]; print version >= '2.1.0'"`
...
to something else than an empty string.
	...
	if test -n "$1"; then
		...
		ac_supports_python_ver=`$PYTHON -c "import sys, string; version = string.split(sys.version)[[0]]; print version >= $1"`
		if test "$ac_supports_python_ver" = "True"; then
	   	   AC_MSG_RESULT([yes])
		else
			AC_MSG_RESULT([no])

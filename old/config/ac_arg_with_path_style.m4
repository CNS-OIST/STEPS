...
dnl     is numeric, where the basetype is encoded as 16 = dos/win, 32 = unix, 
dnl     64 = url/www, 0 = other.
dnl 
...
dnl 
dnl     The macro provides a configure' --with-path-style option that can be 
dnl     used with descriptive arg names. If not explicitly given, the 
dnl     $target_os will be checked to provide a sane default. Additional 
dnl     (lower) bits can be used by the user for some additional magic, higher 
dnl     bits are reserved for this macro.
dnl 
dnl     The mnemonic "strict" or "also" is used to instruct the code that 
dnl     additional seperators shall be accepted but converted to the separator 
dnl     of the underlying pathstyle system. (or-512)
dnl
...
dnl ----------------------------------------------------------------------------
dnl Authors: 
dnl     Guido Draheim <guidod@gmx.de>
dnl ----------------------------------------------------------------------------
dnl Source: 
dnl     http://autoconf-archive.cryp.to/ac_arg_with_path_style.html
dnl ----------------------------------------------------------------------------

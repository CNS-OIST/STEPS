...
std::vector<ComplexSReac*> Surfsys::getAllComplexSReacs() const {
    std::vector<ComplexSReac*> cplxreacs;
    cplxreacs.reserve(pComplexSReacs.size());
    for (auto const& cplxreac: pComplexSReacs) {
...
std::vector<Spec*> Surfsys::getAllSpecs() const {
    std::set<Spec*, util::DerefPtrLess<Spec>> specs_set;
    for (auto const& sreac: getAllSReacs()) {
    ...
    for (auto const& cplxsreac: getAllComplexSReacs()) {
...
void Surfsys::_handleComplexSReacAdd(ComplexSReac& sreac) {
    AssertLog(&sreac.getSurfsys() == this);
    _checkSReacID(sreac.getID());
...
void Surfsys::_handleSpecDelete(Spec& spec) {
    ...
    }
    {
        ...
        for (auto const& oc: pOhmicCurrs) {
            Spec& cstate = oc.second->getChanState();
            if (&cstate == &spec) {
...
void Surfsys::_handleChanDelete(Chan& chan) {
    {
        ...
        for (auto const& ohmcurr: pOhmicCurrs) {
            Chan& chans = ohmcurr.second->getChanState().getChan();
            if (&chans == &chan) {
    ...
    }
    {
        ...
        for (auto const& ghkcurr: pGHKcurrs) {
            Chan& chans = ghkcurr.second->getChanState().getChan();
            if (&chans == &chan) {
...
std::vector<VDepSReac*> Surfsys::getAllVDepSReacs() const {
    std::vector<VDepSReac*> vdepsreac;
...
ComplexSReac& Surfsys::_getComplexSReac(uint lidx) const {
    AssertLog(lidx < pComplexSReacs.size());
    auto rc_it = pComplexSReacs.begin();

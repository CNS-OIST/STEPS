...
Diff::Diff(std::string const& id, Volsys* volsys, Spec* lig, double dcst)
    ...
    , pDcst(dcst)
    , pIsvolume(true) {
    ArgErrLogIf(pVolsys == nullptr, "No volsys provided to Diff initializer function.");
...
Diff::Diff(std::string const& id, Surfsys* surfsys, Spec* lig, double dcst)
    ...
    , pDcst(dcst)
    , pIsvolume(false) {
    ArgErrLogIf(pSurfsys == nullptr, "No surfsys provided to Diff initializer function.");

...
#include "exocytosis.hpp"
#include "linkspec.hpp"
#include "model.hpp"
#include "spec.hpp"
#include "vessdiff.hpp"
...
std::vector<VesSReac*> VesSurfsys::getAllVesSReacs() const {
    std::vector<VesSReac*> vessreacs;
    vessreacs.reserve(pVesSReacs.size());

    for (auto const& vsr: pVesSReacs) {
        vessreacs.push_back(vsr.second);
    }
    return vessreacs;
}
...
std::vector<Exocytosis*> VesSurfsys::getAllExocytosis() const {
    std::vector<Exocytosis*> exos;
    exos.reserve(pExocytosis.size());

    for (auto const& exo: pExocytosis) {
        exos.push_back(exo.second);
    }
    return exos;
}
...
util::flat_set<Spec*> VesSurfsys::getAllSpecs() const {

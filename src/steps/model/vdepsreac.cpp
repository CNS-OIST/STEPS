...
VDepSReac::VDepSReac(std::string const& id,
    ...
    ArgErrLogIf(pSurfsys == nullptr, "No surfsys provided to SReac initializer function");
    ...
    pK.resize(pTablesize);
    std::memcpy(pK.data(), ktab.data(), pTablesize * sizeof(double));
...
std::vector<Spec*> VDepSReac::getAllSpecs() const {
    SpecPVec specs;
    bool first_occ;

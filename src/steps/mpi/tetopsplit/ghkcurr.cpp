...
void GHKcurr::setupDeps() {
    ...
    if (itet->getHost() != pTri->getHost()) {
        ...
        os << "Patch triangle " << pTri->idx() << " and its compartment tetrahedron " << itet->idx()
...
double GHKcurr::rate(mpi::tetopsplit::TetOpSplitP* solver) {
...
void GHKcurr::apply(const rng::RNGptr& /*rng*/, double /*dt*/, double /*simtime*/, double period) {
    WmVol* itet = pTri->iTet();
    WmVol* otet = pTri->oTet();

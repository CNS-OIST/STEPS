...
template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setPatchCount(std::string const& p,
                                                  std::string const& s,
                                                  double n,
                                                  const math::DistributionMethod distribution) {
...
template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getVertIClamp(const osh::GO vert) const {
    return sim->getVertIClamp(mesh::vertex_global_id_t(vert));
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setVertIClamp(const osh::GO vert, const double current) {
    sim->setVertIClamp(mesh::vertex_global_id_t(vert), current);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTetCount(osh::GO tet, const std::string& s) const {
    double res;
    getBatchTetCountsNP(&tet, 1, s, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTetConc(osh::GO tet, const std::string& s) const {
    double res;
    getBatchTetConcsNP(&tet, 1, s, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setTetCount(osh::GO tet, const std::string& s, double count) {
    setBatchTetCountsNP(&tet, 1, s, &count, 1);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setTetConc(osh::GO tet, const std::string& s, double conc) {
    setBatchTetConcsNP(&tet, 1, s, &conc, 1);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTriCount(osh::GO tri, const std::string& s) const {
    double res;
    getBatchTriCountsNP(&tri, 1, s, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setTriCount(osh::GO tri, const std::string& s, double count) {
    setBatchTriCountsNP(&tri, 1, s, &count, 1);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getVertV(osh::GO vert) const {
    double res;
    getBatchVertVsNP(&vert, 1, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTriV(osh::GO tri) const {
    double res;
    getBatchTriVsNP(&tri, 1, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTetV(osh::GO tet) const {
    double res;
    getBatchTetVsNP(&tet, 1, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTriOhmicI(osh::GO tri, const std::string& curr) const {
    double res;
    getBatchTriOhmicIsNP(&tri, 1, curr, &res, 1);
    return res;
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
double TetOpSplit<SSA, SearchMethod>::getTriGHKI(osh::GO tri, const std::string& curr) const {
    double res;
    getBatchTriGHKIsNP(&tri, 1, curr, &res, 1);
    return res;
}
...
template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setBatchTetCounts(const std::vector<osh::GO>& tets,
                                                      std::string const& s,
...
template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setBatchTetConcs(const std::vector<osh::GO>& tets,
                                                     std::string const& s,
...
template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setBatchTriCounts(const std::vector<osh::GO>& tris,
                                                      std::string const& s,
                                                      std::vector<double>& counts) {
...
template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::getBatchTetCountsNP(const osh::GO* indices,
                                                        size_t input_size,
                                                        std::string const& s,
                                                        double* counts,
    ...
    (void) output_size;
    sim->getBatchElemValsNP(indices, input_size, steps::dist::model::species_name(s), counts);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::getBatchTetConcsNP(const osh::GO* indices,
                                                       size_t input_size,
                                                       std::string const& s,
                                                       double* concs,
                                                       size_t output_size) const {
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchElemValsNP(indices, input_size, steps::dist::model::species_name(s), concs, true);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setBatchTetCountsNP(const osh::GO* indices,
                                                        size_t input_size,
                                                        std::string const& s,
                                                        double* counts,
                                                        size_t output_size) {
    assert(input_size == output_size);
    (void) output_size;
    sim->setBatchElemValsNP(indices, input_size, steps::dist::model::species_name(s), counts);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setBatchTetConcsNP(const osh::GO* indices,
                                                       size_t input_size,
                                                       std::string const& s,
                                                       double* concs,
                                                       size_t output_size) {
    assert(input_size == output_size);
    (void) output_size;
    sim->setBatchElemValsNP(indices, input_size, steps::dist::model::species_name(s), concs, true);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::getBatchTriCountsNP(const osh::GO* indices,
                                                        size_t input_size,
                                                        std::string const& s,
                                                        double* counts,
                                                        size_t output_size) const {
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchBoundCountNP(indices, input_size, steps::dist::model::species_name(s), counts);
}

template <steps::dist::SSAMethod SSA, steps::dist::NextEventSearchMethod SearchMethod>
void TetOpSplit<SSA, SearchMethod>::setBatchTriCountsNP(const osh::GO* indices,
                                                        size_t input_size,
                                                        std::string const& s,
                                                        double* counts,
                                                        size_t output_size) {
    assert(input_size == output_size);
    (void) output_size;
    sim->setBatchBoundCountNP(indices, input_size, steps::dist::model::species_name(s), counts);
}
...
void TetOpSplit<SSA, SearchMethod>::getBatchVertVsNP(const osh::GO* indices,
    ...
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchVertVsNP(indices, input_size, voltages);
}
...
void TetOpSplit<SSA, SearchMethod>::getBatchTriVsNP(const osh::GO* indices,
    ...
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchTriVsNP(indices, input_size, voltages);
}
...
void TetOpSplit<SSA, SearchMethod>::getBatchTetVsNP(const osh::GO* indices,
    ...
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchTetVsNP(indices, input_size, voltages);
}
...
void TetOpSplit<SSA, SearchMethod>::getBatchTriOhmicIsNP(const osh::GO* indices,
    ...
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchTriOhmicIsNP(indices,
                              input_size,
...
void TetOpSplit<SSA, SearchMethod>::getBatchTriGHKIsNP(const osh::GO* indices,
    ...
    assert(input_size == output_size);
    (void) output_size;
    sim->getBatchTriGHKIsNP(indices,
                            input_size,
                            steps::dist::model::ghk_current_id(curr),
                            currents);
}

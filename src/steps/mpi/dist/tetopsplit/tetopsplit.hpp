...
class TetOpSplitBase {
  public:
    ...
    virtual double getVertIClamp(osh::GO vert) const = 0;
    virtual void setVertIClamp(osh::GO vert, double current) = 0;

    virtual double getTetCount(osh::GO tet, const std::string& s) const = 0;
    virtual double getTetConc(osh::GO tet, const std::string& s) const = 0;
    virtual void setTetCount(osh::GO tet, const std::string& s, double count) = 0;
    virtual void setTetConc(osh::GO tet, const std::string& s, double conc) = 0;
    virtual double getTriCount(osh::GO tri, const std::string& s) const = 0;
    virtual void setTriCount(osh::GO tri, const std::string& s, double count) = 0;
    virtual double getVertV(osh::GO vert) const = 0;
    virtual double getTriV(osh::GO tet) const = 0;
    virtual double getTetV(osh::GO tet) const = 0;
    virtual double getTriOhmicI(osh::GO tet, const std::string& curr) const = 0;
    virtual double getTriGHKI(osh::GO tet, const std::string& curr) const = 0;

    virtual std::vector<double> getBatchTetCounts(const std::vector<osh::GO>& tets,
                                                  std::string const& s) const = 0;
    virtual std::vector<double> getBatchTetConcs(const std::vector<osh::GO>& tets,
                                                 std::string const& s) const = 0;
    virtual void setBatchTetCounts(const std::vector<osh::GO>& tets,
                                   std::string const& s,
                                   std::vector<double>& counts) = 0;
    virtual void setBatchTetConcs(const std::vector<osh::GO>& tets,
                                  std::string const& s,
                                  std::vector<double>& concs) = 0;
    virtual std::vector<double> getBatchTriCounts(const std::vector<osh::GO>& tris,
                                                  std::string const& s) const = 0;
    virtual void setBatchTriCounts(const std::vector<osh::GO>& tris,
                                   std::string const& s,
                                   std::vector<double>& counts) = 0;
    virtual void getBatchTetCountsNP(const osh::GO* indices,
                                     size_t input_size,
                                     std::string const& s,
                                     double* counts,
                                     size_t output_size) const = 0;
    virtual void getBatchTetConcsNP(const osh::GO* indices,
                                    size_t input_size,
                                    std::string const& s,
                                    double* concs,
                                    size_t output_size) const = 0;
    virtual void setBatchTetCountsNP(const osh::GO* indices,
                                     size_t input_size,
                                     std::string const& s,
                                     double* counts,
                                     size_t output_size) = 0;
    virtual void setBatchTetConcsNP(const osh::GO* indices,
                                    size_t input_size,
                                    std::string const& s,
                                    double* concs,
                                    size_t output_size) = 0;
    virtual void getBatchTriCountsNP(const osh::GO* indices,
                                     size_t input_size,
                                     std::string const& s,
                                     double* counts,
                                     size_t output_size) const = 0;
    virtual void setBatchTriCountsNP(const osh::GO* indices,
                                     size_t input_size,
                                     std::string const& s,
                                     double* counts,
                                     size_t output_size) = 0;
    ...
    virtual void setEfieldTolerances(double atol, double rtol, KSPNormType norm_type) = 0;
#endif // USE_PETSC
...
class TetOpSplit : public TetOpSplitBase {
  public:
    ...
    double getCompCount(std::string const & c, std::string const & s) const override;
    void setCompCount(std::string const& c,
                      std::string const& s,
                      double n,
                      const math::DistributionMethod distribution) override;

    double getCompConc(std::string const & c, std::string const & s) const override;
    void setCompConc(std::string const& c,
                     std::string const& s,
                     double conc,
                     const math::DistributionMethod distribution) override;
    ...
    double getPatchCount(std::string const & p, std::string const & s) const override;

    void setPatchCount(std::string const& p,
                       std::string const& s,
                       double n,
                       const math::DistributionMethod distribution) override;
    ...
    double getVertIClamp(const osh::GO vert) const override;

    void setVertIClamp(const osh::GO vert, const double value) override;

    double getTetCount(osh::GO tet, const std::string& s) const override;

    double getTetConc(osh::GO tet, const std::string& s) const override;

    void setTetCount(osh::GO tet, const std::string& s, double count) override;

    void setTetConc(osh::GO tet, const std::string& s, double conc) override;

    double getTriCount(osh::GO tri, const std::string& s) const override;

    void setTriCount(osh::GO tri, const std::string& s, double count) override;

    double getVertV(osh::GO vert) const override;

    double getTriV(osh::GO tet) const override;

    double getTetV(osh::GO tet) const override;

    double getTriOhmicI(osh::GO tet, const std::string& curr) const override;

    double getTriGHKI(osh::GO tet, const std::string& curr) const override;
    ...
    void setBatchTetCounts(const std::vector<osh::GO>& tets,
                           std::string const& s,
                           std::vector<double>& counts) override;

    void setBatchTetConcs(const std::vector<osh::GO>& tets,
                          std::string const& s,
                          std::vector<double>& concs) override;
    ...
    void getBatchTetCountsNP(const osh::GO* indices,
                             size_t input_size,
                             std::string const& s,
                             double* counts,
                             size_t output_size) const override;

    void getBatchTetConcsNP(const osh::GO* indices,
                            size_t input_size,
                            std::string const& s,
                            double* concs,
                            size_t output_size) const override;

    void setBatchTetCountsNP(const osh::GO* indices,
                             size_t input_size,
                             std::string const& s,
                             double* counts,
                             size_t output_size) override;

    void setBatchTetConcsNP(const osh::GO* indices,
                            size_t input_size,
                            std::string const& s,
                            double* concs,
                            size_t output_size) override;

    void getBatchTriCountsNP(const osh::GO* indices,
                             size_t input_size,
                             std::string const& s,
                             double* counts,
                             size_t output_size) const override;

    void setBatchTriCountsNP(const osh::GO* indices,
                             size_t input_size,
                             std::string const& s,
                             double* counts,
                             size_t output_size) override;
    ...
    void setDiffBoundaryDiffusionActive(const std::string &name, const std::string &spec, bool active) override;

    bool getDiffBoundaryDiffusionActive(const std::string &name, const std::string &spec) override;
    ...
    void setEfieldTolerances(double atol, double rtol, KSPNormType norm_type) override;
#endif // USE_PETSC
    ...
    double getEFieldTime() const noexcept override { return sim->getElapsedEField(); }
    double getRDTime() const noexcept override { return sim->getElapsedDiff() + sim->getElapsedSSA(); }
    double getDiffusionTime() const noexcept  override { return sim->getElapsedDiff(); }
    double getReactionTime() const noexcept override { return sim->getElapsedSSA(); }
    double getDataExchangeTime() const noexcept override { return dataexchangetime; }

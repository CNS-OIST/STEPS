...
void OmegaHSimulation<SSA, SearchMethod>::setMembRes(const model::membrane_id& membrane,
    ...
    this->statedef->setResistance(membrane, resistance);
    this->statedef->setReversalPotential(membrane, reversal_potential);
}

template <SSAMethod SSA, NextEventSearchMethod SearchMethod>
std::pair<osh::Real, osh::Real> OmegaHSimulation<SSA, SearchMethod>::getMembRes(
    const model::membrane_id& membrane) {
    return {statedef->getResistance(membrane), statedef->getReversalPotential(membrane)};
}

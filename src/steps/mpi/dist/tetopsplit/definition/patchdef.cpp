...
container::species_id Patchdef::getSpecPatchIdx(model::species_id species) const {
      ...
      throw std::logic_error("Unregistered species id " +
                                   pStatedef_.getSpecID(species));
    }

//-------------------------------------------------------

Patchdef::Patchdef(
    const Statedef &statedef, model::patch_id t_model_patch,
    ...
    : pStatedef_(statedef), model_patch_(t_model_patch),
      inner_compartment_id_(inner_compartment_id),
      outer_compartment_id_(outer_compartment_id),
      container_patch_id_(container_patch_id), nKProcs_(0) {}
...
container::surface_reaction_id Patchdef::addSurfaceReacImpl(
    ...
    const std::vector<container::species_id> &products_o, PropensityType kcst) {
  typename SReacdefBase<PropensityType>::SurfaceReactionComponents
      reaction_components;
  reaction_components.reserve(reactants_i.size() + reactants_s.size() +
                              reactants_o.size() + products_i.size() +
                              products_s.size() + products_o.size());

  // prepare arguments for building SReacdef
  auto accum_reaction_comps = [&reaction_components,
                               this](const auto loc, const auto classifier,
                                     const auto &vec) {
    return std::transform(
        vec.begin(), vec.end(), std::back_inserter(reaction_components),
  ...
  accum_reaction_comps(
      SReacdefBase<PropensityType>::SpecieLocation::InnerCompartment,
      SReacdefBase<PropensityType>::SpecieClassifier::Reactant, reactants_i);
  accum_reaction_comps(SReacdefBase<PropensityType>::SpecieLocation::Patch,
                       ...
                       reactants_s);
  accum_reaction_comps(
      SReacdefBase<PropensityType>::SpecieLocation::OuterCompartment,
      SReacdefBase<PropensityType>::SpecieClassifier::Reactant, reactants_o);
  accum_reaction_comps(
      SReacdefBase<PropensityType>::SpecieLocation::InnerCompartment,
      SReacdefBase<PropensityType>::SpecieClassifier::Product, products_i);
  accum_reaction_comps(SReacdefBase<PropensityType>::SpecieLocation::Patch,
                       SReacdefBase<PropensityType>::SpecieClassifier::Product,
                       products_s);
  accum_reaction_comps(
      SReacdefBase<PropensityType>::SpecieLocation::OuterCompartment,
      SReacdefBase<PropensityType>::SpecieClassifier::Product, products_o);
  ...
  getContainer<PropensityType>().push_back(
      std::make_unique<SReacdefBase<PropensityType>>(
          pStatedef_, container_patch_id_, container::kproc_id(nKProcs_),
          reac_id, reaction_components, kcst));
...
model::species_id
Patchdef::getSpecModelIdx(container::species_id species) const {
  if (!(species <
        static_cast<container::species_id::value_type>(specC2M_.size()))) {
    throw std::invalid_argument(
        std::string("Unregistered needed species in patch ") + model_patch_);
  }
...
inline kproc::KProcType
Patchdef::getKProcType(container::kproc_id kproc) const {
...
inline osh::I64 Patchdef::getNKProcs() const { return nKProcs_; }
...
template container::surface_reaction_id Patchdef::addSurfaceReacImpl(
    ...
    const std::vector<container::species_id> &product_o, SReacInfo kcst);
...
template container::surface_reaction_id Patchdef::addSurfaceReacImpl(
    ...
    const std::vector<container::species_id> &product_i,
    const std::vector<container::species_id> &product_s,
    const std::vector<container::species_id> &product_o, VDepInfo kcst);
//-------------------------------------------------------
...
template container::surface_reaction_id Patchdef::addSurfaceReacImpl(
    ...
    const std::vector<container::species_id> &product_i,
    const std::vector<container::species_id> &product_s,
    const std::vector<container::species_id> &product_o, GHKInfo kcst);

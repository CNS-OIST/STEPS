...
class Compdef;
class Diffdef;
template <typename P> class SReacdefBase;
struct SReacInfo {
  osh::Real kCst;
};
...
struct GHKInfo {
  ...
  /// if true, the surface reaction involves an ion transfer from the inner to outer compartment, and conversely otherwise.
  bool in2out;

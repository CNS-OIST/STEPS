...
Statedef::Statedef(const steps::model::Model &model,
                   const steps::dist::DistMesh &mesh) {

    // Species
    auto allSpecs = model.getAllSpecs();
    std::sort(allSpecs.begin(), allSpecs.end(),
              stepsObjComp<steps::model::Spec>);
    ...
    for (auto *comp : allComps) {
        ...
        std::sort(allReacs.begin(), allReacs.end(),
                  stepsObjComp<steps::model::Reac>);
    ...
    std::sort(allPatches.begin(), allPatches.end(),
              stepsObjComp<steps::dist::DistPatch>);
    for (auto *patch : allPatches) {
        model::patch_id patch_id{patch->getID()};
        const model::compartment_id inner_compartment{
            patch->getIComp()->getID()};
        const auto ocomp = patch->getOComp();
        boost::optional<model::compartment_id> outer_compartment = boost::none;
        if (ocomp != nullptr) {
            outer_compartment = model::compartment_id{ocomp->getID()};
        }

        addPatch(patch_id, inner_compartment, outer_compartment);

        auto allPatchSpecs = patch->getAllSpecs(&model);
        std::sort(allPatchSpecs.begin(), allPatchSpecs.end(),
                  stepsObjComp<steps::model::Spec>);
        ...
        std::sort(allSReacs.begin(), allSReacs.end(),
                  stepsObjComp<steps::model::SReac>);
        for (auto *sreac : allSReacs) {
            std::vector<model::species_name> lhs_i, lhs_s, lhs_o, rhs_i, rhs_s,
                rhs_o;
            ...
            surfReacIdxs[model::surface_reaction_id(sreac->getID())] =
                addSurfReac(patch_id, lhs_i, lhs_s, lhs_o, rhs_i, rhs_s, rhs_o,
                            sreac->getKcst());
        }
        ...
        for (auto &ssysName : patch->getSurfsys()) {
            ...
            if (not ssys->_getAllDiffs().empty()) {
                throw std::logic_error(
                    "Model contains surface diffusion rules.");
            }
            // Voltage dependent surface reactions
            for (auto &vdepsreac : ssys->_getAllVDepSReacs()) {
                std::vector<model::species_name> lhs_i, lhs_s, lhs_o, rhs_i,
                    rhs_s, rhs_o;
                ...
                double *kTable = vdepsreac.second->_getK();
                ...
                std::function<osh::Real(osh::Real)> kcst = [vmin, vmax, dv, kTable](double v) {
                    if (v > vmax) {
                        ...
                        msg << "Voltage is higher than maximum: " << v << " > "
                            << vmax;
                        throw std::out_of_range(msg.str());
                    }
                    if (v < vmin) {
                        std::ostringstream msg;
                        msg << "Voltage is lower than minimum: " << v << " < "
                            << vmin;
                        throw std::out_of_range(msg.str());
                    }
                    double v2 = ((v - vmin) / dv);
                ...
                addVDepSurfReac(patch_id, lhs_i, lhs_s, lhs_o, rhs_i, rhs_s,
                                rhs_o, kcst);
            }
        }
    }
        ...
        for (auto &patchId : memb.second->patches()) {
            ...
            for (auto &ssysName : patch->getSurfsys()) {
                ...
                for (auto &curr : ssys->_getAllGHKcurrs()) {
                    ...
                    auto *chanstate = curr.second->getChanState();
                    boost::optional<osh::Real> voconc(boost::none);
...
container::patch_id Statedef::addPatch(
    ...
    patchdefPtrs.emplace_back(
        std::make_unique<Patchdef>(*this, patchId, container_id,
                                   inner_compartment_id, outer_compartment_id));
    patchModelIdxs[patchId] = container_id;
    return container_id;
}

container::surface_reaction_id
Statedef::addSurfReac(const model::patch_id &patchId,
  ...
  SReacInfo info;
  info.kCst = kcst;
  return addSurfReacImpl<SReacInfo>(patchId, reactants_i, reactants_s,
                                    reactants_o, products_i, products_s,
                                    products_o, info);
}

container::surface_reaction_id
Statedef::addVDepSurfReac(const model::patch_id &patchId,
                          const std::vector<model::species_name> &reactants_i,
                          const std::vector<model::species_name> &reactants_s,
                          const std::vector<model::species_name> &reactants_o,
                          const std::vector<model::species_name> &products_i,
  ...
  VDepInfo info;
  info.kCstFun = kcst;
  return addSurfReacImpl(patchId, reactants_i, reactants_s, reactants_o,
                         products_i, products_s, products_o, info);
}
...
void Statedef::addGHKCurrentSurfReac(const model::ghk_current_id& curr_id,
                                     ...
                                     osh::I64 valence,
                                     boost::optional<osh::Real> outer_conc,
                                     boost::optional<osh::Real> inner_conc) {
    ...
    if (it != membranePtrs.end()) {
        ...
        if (it_channel != memb.channels().end()) {
            ...
            info.inner_conc = inner_conc ? boost::optional<double>(*inner_conc / m3_per_liter *
                                                                   math::AVOGADRO)
                                         /*[m^{-3}]*/
                                         : boost::none;
            info.outer_conc = outer_conc ? boost::optional<double>(*outer_conc / m3_per_liter *
                                                                   math::AVOGADRO)
                                         : boost::none;
...
container::surface_reaction_id
Statedef::addSurfReacImpl(const model::patch_id &patchId,
                          const std::vector<model::species_name> &reactants_i,
                          const std::vector<model::species_name> &reactants_s,
                          const std::vector<model::species_name> &reactants_o,
                          const std::vector<model::species_name> &products_i,
  ...
  Patchdef &patch =
      *patchdefPtrs[static_cast<size_t>(patchModelIdxs[patchId].get())];

  const Compdef &inner_compartment = *compdefPtrs[static_cast<size_t>(
      compModelIdxs[patch.getInnerCompId()].get())];
  boost::optional<const Compdef &> outer_compartment =
      patch.getOuterCompId()
          ? *compdefPtrs[static_cast<size_t>(
                compModelIdxs[*patch.getOuterCompId()].get())]
          : boost::optional<const Compdef &>();

  auto &specMdlIdxs = this->specModelIdxs;
  auto convert_id_type_patch =
      [&patch, &specMdlIdxs](const std::vector<model::species_name> &v) {
  ...
  auto convert_id_type_comp =
      [&specMdlIdxs](const Compdef &comp,
        ...
        std::transform(v.begin(), v.end(), std::back_inserter(res), [&specMdlIdxs, &comp](auto& l) {
            ...
            if (id.unknown()) {
              std::ostringstream msg;
              msg << "Compartment " << comp.getID() << ": unknown species "
                  << l;
  ...
  auto id = patch.addSurfaceReacImpl<PropensityType>(
      ...
      !reactants_o.empty()
          ? convert_id_type_comp(*outer_compartment, reactants_o)
          : empty,
      convert_id_type_comp(inner_compartment, products_i),
      convert_id_type_patch(products_s),
      !products_o.empty() ? convert_id_type_comp(*outer_compartment, products_o)
                          : empty,
      kcst);
  return id;
}
...
model::species_id
Statedef::addCompSpec(const model::compartment_id &compartment,
                      const model::species_name &species) {
  auto spec_id = addSpec(species);
  auto &compdef =
      compdefPtrs[static_cast<size_t>(compModelIdxs[compartment].get())];
  compdef->addSpec(spec_id);
  return spec_id;
}

std::vector<model::species_id>
Statedef::addCompSpecs(const model::compartment_id &compartment,
...
std::vector<model::species_id>
Statedef::addPatchSpecs(const model::patch_id &patchId,
                        const std::vector<model::species_name> &species) {
  std::vector<model::species_id> ids;
  ids.reserve(species.size());
  for (const auto &spec_id : species) {
...
model::species_id Statedef::addPatchSpec(const model::patch_id &patch_id,
  ...
  auto spec_id = addSpec(species);
  auto &patchdef =
      patchdefPtrs[static_cast<size_t>(patchModelIdxs[patch_id].get())];
  patchdef->addSpec(spec_id);
  return spec_id;
}

container::species_id
Statedef::getCompSpecContainerIdx(const model::compartment_id &compartment,
  ...
  return compdefPtrs[static_cast<size_t>(comp_model_idx.get())]
      ->getSpecContainerIdx(spec_model_idx);
}

container::diffusion_id
Statedef::addCompDiff(const model::compartment_id &compartment,
                      const model::species_name &species_name, osh::Real dcst) {
  const auto scidx = getCompSpecContainerIdx(compartment, species_name);
  return compdefPtrs[static_cast<size_t>(compModelIdxs[compartment].get())]
      ->addDiff(scidx, dcst);
}

container::reaction_id
Statedef::addCompReac(const model::compartment_id &compartment,
  ...
  std::transform(reactants.begin(), reactants.end(),
...
Statedef::getSReacIdx(const model::surface_reaction_id &reac) const {
    auto srit = surfReacIdxs.find(reac);
    if (srit != surfReacIdxs.end()) {
        return srit->second;
    } else {
        throw std::invalid_argument("Unregistered surface reaction " + reac);
    }
}
...
void Statedef::addOhmicCurrent(
    const model::ohmic_current_id &curr_id, const model::membrane_id &membrane,
    ...
    const auto membrane_it = membranePtrs.find(membrane);
    if (membrane_it == membranePtrs.end()) {
        std::ostringstream oss;
        oss << "Unknown membrane " << membrane;
        throw std::logic_error(oss.str());
    }
    auto &memb = membrane_it->second;
    const auto channel_it = memb->channels().find(channel);
    if (channel_it == memb->channels().end()) {
        std::ostringstream oss;
        oss << "Unknown channel " << channel;
        throw std::logic_error(oss.str());
    }
    const auto &patch_id = memb->getPatch();
    const auto &patch = getPatchdef(patch_id);
        ...
        curr_it =
            ohmicCurrPtrs
                .emplace(curr_id, std::make_unique<OhmicCurrent>(
                                      conductance, reversal_potential, spec))
                .first;
    }
    channel_it->second.addOhmicCurrent(*curr_it->second);
}

void Statedef::addCompartmentConductivity(
    const model::compartment_id &compartment_id, const osh::Real conductivity) {
  compartment_conductivity_[compartment_id] = conductivity;
}

void Statedef::addChannel(
    const model::membrane_id &membrane, const model::channel_id &channel,
  ...
  const auto &patch_id = membrane_it->second->getPatch();
  const auto &patch = getPatchdef(patch_id);
  std::vector<container::species_id> specs;
  specs.reserve(channel_states.size());
  for (const auto &channel_state : channel_states) {
    specs.push_back(
        patch.getSpecPatchIdx(addPatchSpec(patch_id, channel_state)));
  }
  membrane_it->second->addChannel(channel, Channel(specs));
}

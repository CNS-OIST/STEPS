...
SReacdefBase<PropensityInfo>::SReacdefBase(
    const Statedef &state_def, const container::patch_id &patch_id,
    container::kproc_id kproc_id,
    container::surface_reaction_id surface_reaction_id,
    const SurfaceReactionComponents &reaction_definition, PropensityInfo info)
    : state_def_(state_def), patch_id_(patch_id), kproc_id_(kproc_id),
      surface_reaction_(surface_reaction_id),
      reaction_components_(reaction_definition), info_(info) {
  container::species_id species_id;
  SpecieClassifier speciesClassifier;
  SpecieLocation surfaceReactionSpecieLocation;
  order_ = 0;
  for (const auto &t : reaction_components_) {
    std::tie(species_id, speciesClassifier, surfaceReactionSpecieLocation) = t;
    if (speciesClassifier == SpecieClassifier::Reactant) {
      order_++;
    }
    auto populateStoichiometry =
        [](std::map<container::species_id, osh::LO> &lhs,
           std::map<container::species_id, osh::LO> &upd,
           SpecieClassifier classifier, container::species_id species) {
    ...
    if (surfaceReactionSpecieLocation == SpecieLocation::InnerCompartment) {
      populateStoichiometry(inner_lhs_, inner_update_, speciesClassifier,
                            species_id);
    }
    if (surfaceReactionSpecieLocation == SpecieLocation::Patch) {
      populateStoichiometry(patch_lhs_, patch_update_, speciesClassifier,
                            species_id);
    }
    if (surfaceReactionSpecieLocation == SpecieLocation::OuterCompartment) {
      populateStoichiometry(outer_lhs_, outer_update_, speciesClassifier,
                            species_id);
    }
  }
  ...
  if (!(all_inner || all_outer) &&
    ...
    throw std::logic_error("A surface reaction involves volume reactants "

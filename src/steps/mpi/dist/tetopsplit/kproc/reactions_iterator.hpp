...
template <typename ReactionClass> struct reactions_iterator;

template <typename ReactionClass> struct reaction_partial {
  inline reaction_partial(ReactionClass &reactions, size_t pos) noexcept
      : reactions_(reactions), pos_(pos) {}
  ...
  osh::Real computeRate(const MolState<NumMolecules> &mol_state) const
      noexcept {
    return reactions_.computeRate(mol_state, pos_);
  }

  size_t getIndex() const noexcept { return pos_; }

  const std::vector<MolStateElementID> &getMolStateElementsUpdates() const
      noexcept {
    return reactions_.getMolStateElementsUpdates(pos_);
  }

  const std::vector<MolStateElementID> &getPropensityDependency() const
      noexcept {
...
template <typename ReactionClass> struct reactions_iterator {
  ...
  inline reactions_iterator operator++(int) noexcept;
...
inline reactions_iterator<ReactionClass>::reactions_iterator(
    ReactionClass &reactions, size_t pos) noexcept
    : partial_(reactions, pos) {}

//-------------------------------------------------------------

template <class ReactionClass>
inline reactions_iterator<ReactionClass> &
reactions_iterator<ReactionClass>::operator++() noexcept {
  ++partial_.pos_;
  return *this;
}
...
template <class ReactionClass>
inline reactions_iterator<ReactionClass>
reactions_iterator<ReactionClass>::operator++(int) noexcept {
...
template <class ReactionClass>
inline const reaction_partial<ReactionClass> &
    reactions_iterator<ReactionClass>::operator*() const noexcept {
  return partial_;
}
...
struct iterator_traits<steps::dist::kproc::reactions_iterator<ReactionClass>> {
  ...
  using value_type =
      typename steps::dist::kproc::reaction_partial<ReactionClass>;
  using pointer =
      typename steps::dist::kproc::reaction_partial<ReactionClass> *;
  using reference =
      typename steps::dist::kproc::reaction_partial<ReactionClass> &;

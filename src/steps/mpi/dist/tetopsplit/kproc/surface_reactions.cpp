...
SurfaceReactionsBase<PropensityType>::updateMolStateAndOccupancy(MolState& mol_state,
    ...
    for (size_t k = 0; k < mol_state_elements.size(); ++k) {
        ...
        assert(mol_state(elmt_id) <= std::numeric_limits<molecules_t>::max() -
                                         std::max(static_cast<molecules_t>(upd[k]), {}));
        assert(mol_state(elmt_id) >= -static_cast<molecules_t>(upd[k]));
        mol_state.add_and_update_occupancy(elmt_id, static_cast<molecules_t>(upd[k]), event_time);
    }
...
template <typename PropensityType>
osh::Real SurfaceReactionsBase<PropensityType>::kinConstantGeomFactor(const DistMesh& mesh,
...
//-------------------------------------------------------
template <typename PropensityType>
osh::Real SurfaceReactionsBase<PropensityType>::computeRate(const MolState& mol_state,
                                                            size_t reaction_index) const {
...
SurfaceReactionsBase<PropensityType>::reactionMolStateDependencyAndStoichiometry(
    ...
    const size_t size{
        ...
        reacdef.template getStoichiometry<PoolChangeTy, SL::InnerCompartment>().size() +
        reacdef.template getStoichiometry<PoolChangeTy, SL::OuterCompartment>().size()};
    ...
    const auto& mol_changes_patch = reacdef.template getStoichiometry<PoolChangeTy, SL::Patch>();
...
template <>
std::string SurfaceReactionsBase<SReacInfo>::name() {
    return "SurfaceReaction";
}

template <>
std::string SurfaceReactionsBase<VDepInfo>::name() {
    return "VDepSurfaceReaction";
}

template <>
std::string SurfaceReactionsBase<GHKInfo>::name() {
...
void SurfaceReactions::updateCcst(DistMesh& mesh) {
    for (size_t k = 0; k < size(); k++) {
        ccsts_[k] = kinConstantGeomFactor(mesh, k) * reacdefs_[k].get().getInfo().kCst;

        // If you reach these asserts probably your mesh is too big/too small or
        // kcst is an invalid value
        assert(ccsts_[k] != std::numeric_limits<double>::infinity());
        assert(ccsts_[k] >= 0);
    }
}

//-------------------------------------------------------
SurfaceReactions::SurfaceReactions(const Statedef& statedef, DistMesh& mesh, MolState& mol_state)
...
void VDepSurfaceReactions::kCstUpdate(osh::Reals& potential_on_verts) {
    for (size_t reaction_idx = 0; reaction_idx < size(); reaction_idx++) {
        const vdep_propensity_fun_t& f = reacdefs_[reaction_idx].get().getInfo().kCstFun;
        const auto& verts =
            Omega_h::gather_verts<DistMesh::dim()>(tri2verts_, boundary_id_[reaction_idx].get());
        ...
        ccsts_[reaction_idx] = kinConstantGeomFactor_[reaction_idx] * f(avg_potential);
        ...
        // sense
        assert(ccsts_[reaction_idx] != std::numeric_limits<double>::infinity());
        assert(ccsts_[reaction_idx] >= 0);
    }
}
    ...
    double rate =
        (std::abs(nuFoRT) > std::numeric_limits<double>::epsilon())
            ? info.permeability * nuFoRT * (conc_i - conc_o * eNuFoRT) / (1.0 - eNuFoRT) *
                  static_cast<double>(mol_state(reaction_lhs[0]))  // number of channels: [1]
    ...
    for (uint ridx = 0; ridx < size(); ++ridx) {
        ...
        if (tri2reac_it == curr2tri2reac_.end()) {
            tri2reac_it =
                curr2tri2reac_
                    .emplace(curr_id,
                             osh::Write<osh::GO>(mesh.owned_bounds_mask().size() * rpt(), -1))
                    .first;
        }
        bool added{false};
...
const osh::Write<osh::GO>& GHKSurfaceReactions::getTri2Curr(
    const model::ghk_current_id& curr_id) const {
...
template osh::Real SurfaceReactionsBase<SReacInfo>::computeRate(const MolState& mol_state,
                                                                size_t index) const;
template osh::Real SurfaceReactionsBase<VDepInfo>::computeRate(const MolState& mol_state,
                                                               size_t index) const;
template osh::Real SurfaceReactionsBase<GHKInfo>::computeRate(const MolState& mol_state,
                                                              size_t index) const;

template osh::Real SurfaceReactionsBase<SReacInfo>::kinConstantGeomFactor(const DistMesh& mesh,
                                                                          size_t index) const;

template const std::vector<MolStateElementID>&
SurfaceReactionsBase<SReacInfo>::updateMolStateAndOccupancy(MolState& mol_state,
                                                            size_t index,
                                                            const osh::Real event_time) const;
template const std::vector<MolStateElementID>&
SurfaceReactionsBase<VDepInfo>::updateMolStateAndOccupancy(MolState& mol_state,
                                                           size_t index,
                                                           const osh::Real event_time) const;
template const std::vector<MolStateElementID>&
SurfaceReactionsBase<GHKInfo>::updateMolStateAndOccupancy(MolState& mol_state,
                                                          size_t index,
                                                          const osh::Real event_time) const;

template void SurfaceReactionsBase<VDepInfo>::report(std::ostream& report_stream,
                                                     size_t index) const;
template void SurfaceReactionsBase<SReacInfo>::report(std::ostream& report_stream,
                                                      size_t index) const;

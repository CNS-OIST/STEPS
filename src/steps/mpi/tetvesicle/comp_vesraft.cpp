...
CompVesRaft::CompVesRaft(solver::Compdef* compdef,
    ...
    , pVol(0.0)
    , pMesh(mesh)
    , pRNG(pCompdef->statedef()->rng())  // have to do this because it's const
    , pVesRaft(vesraft) {
...
void CompVesRaft::restore(std::fstream& cp_file) {
    ...
    for (auto const& ves: vesicles) {
        solver::Vesicledef* vesdef = def()->statedef()->vesicledef(ves.first);
        for (auto const& ves_id: ves.second) {
            auto* ves_p = new Vesicle(vesdef, this, ves_id, cp_file);
            std::map<tetrahedron_global_id, double> tets_overlap = ves_p->getOverlap_gidx();
...
bool CompVesRaft::checkPos(vector_t* pos,
    ...
    if (central_tet_idx.valid()) {
        // For now any moves outside of diffusion are compartmentalised i.e.
        // one can only change vesicle position inside a compartment (though they
        // can diffuse across group boundaries). So we currently expect the start tet
        // to be in this compartment.
        auto const search = pTetidcs_G_to_L.find(central_tet_idx);
        if (search == pTetidcs_G_to_L.end()) {
            std::ostringstream os;
            os << "Central tetrahedron in CompVesRaft::checkPos search is outside ";
            os << "compartmental boundaries.\n";
            ProgErrLog(os.str());
        }
        ...
        if (ves->getPosition().dist2(pos2) < diam * diam) {
            for (const auto& itet: ves->tets_overlap()) {
        ...
        if (curr_layer.empty()) {
            ...
            auto walkable = [=](const tetrahedron_global_id& tet) {
                if (pTetidcs_G_to_L.find(tet) == pTetidcs_G_to_L.end()) {
                    // If we are here the tet is in another compartment, or no compartment at all
                    auto const tet_p = pVesRaft->tet_(tet);
                    return tet_p != nullptr and
                           pVesicles_permittedcomps[vesgidx].find(tet_p->getCompVesRaft()) !=
                               pVesicles_permittedcomps[vesgidx].end();
                }
                return true;
            };
...
void CompVesRaft::setVesicleCount(solver::vesicle_global_id vidx, uint count) {
    ...
    solver::Vesicledef* vesdef = def()->statedef()->vesicledef(vidx);
...
void CompVesRaft::applyExocytosis(Vesicle* vesicle, solver::exocytosis_global_id exo_gidx) {
    ...
    solver::Exocytosisdef* exodef = def()->statedef()->exocytosisdef(exo_gidx);
    ...
    // There may be no tet there, null pointer, in which case do nothing
    if (extra_tet != nullptr) {
        for (auto& inner_spec_count: vesicle->getInnerSpecCounts()) {
            ...
            solver::spec_local_id spec_lidx = extra_tet->compdef()->specG2L(spec_gidx);
            ...
            pVesRaft->regTetPoolSync_(extra_tet->idx(), spec_gidx, inner_spec_count.second);
        }
    }

    if (exodef->getKissAndRun()) {
        // In this version of kiss-and-run, some species (optinally) change ID.
...
void CompVesRaft::setVesiclePos(solver::vesicle_global_id vidx,
    ...
    for (auto const& v: pVesicles[vidx]) {
        if (v->getUniqueIndex() == ves_unique_idx) {
            ...
            if (search == pTetidcs_G_to_L.end()) {
                CLOG(WARNING, "general_log")
                    << "Did not change position: outside compartment boundaries.\n";
                return;
            }
            ...
            if (checkPos(&new_pos,
                ...
                if ((!force) || (jama_vesicle == nullptr)) {
                    ...
                    bool moved1 = v->setPosition(new_pos, tets_overlap_new);
                    ...
                    bool moved2 = jama_vesicle->setPosition(old_pos_vect, tets_overlap_prev);

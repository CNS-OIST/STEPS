...
class Vesicle {
  public:
    ...
    inline std::map<tetrahedron_global_id, double>& getOverlap_gidx() noexcept {
    ...
    std::map<tetrahedron_global_id, double>::const_iterator tets_overlap_begin_gidx()
        const noexcept {
        return pTets_overlap_gidx.begin();
    }
    std::map<tetrahedron_global_id, double>::const_iterator tets_overlap_end_gidx() const noexcept {
        return pTets_overlap_gidx.end();
    }
    std::map<tetrahedron_global_id, double> const& tets_overlap() const noexcept {
        return pTets_overlap_gidx;
    }
    ...
    /*
      std::vector<PointSpec *> getSurfSpecs(solver::spec_global_id spec_gidx,

...
solver::vesicle_individual_id TetVesicleVesRaft::_addCompVesicle(solver::comp_global_id cidx,
    ...
    auto& vesdef = statedef().vesicledef(vidx);
...
std::vector<solver::vesicle_individual_id> TetVesicleVesRaft::_getAllVesicleIndices() const {
    std::vector<solver::vesicle_individual_id> ves_indices;
    ves_indices.reserve(pVesicles.size());

    for (auto const& ves: pVesicles) {
        ves_indices.emplace_back(ves.first);
    };
...
std::vector<solver::vesicle_individual_id> TetVesicleVesRaft::_getAllVesicleIndicesOnPath() const {
    std::vector<solver::vesicle_individual_id> ves_indices;

    for (auto const& ves: pVesicles) {
        if (ves.second->onPath()) {
            ves_indices.emplace_back(ves.first);
        }
    };
...
void TetVesicleVesRaft::_setSingleVesiclePos(solver::vesicle_global_id vidx,
                                             solver::vesicle_individual_id ves_unique_index,
    ...
    auto* comp = vesicle_it->second->comp();
...
void TetVesicleVesRaft::addOverlap_(const std::map<tetrahedron_global_id, double>& tets_overlap,
                                    Vesicle* ves) const {
...
void TetVesicleVesRaft::_setSingleRaftRaftEndocytosisK(solver::raft_global_id ridx,
    ...
    const auto& raftendo_def = statedef().raftendocytosisdef(rendoidx);

    const auto raft_it = pRafts.find(raft_unique_index);
    if (raft_it == pRafts.end()) {
        CLOG(WARNING, "general_log") << "Raft unique id " << raft_unique_index << "unknown.\n";
        return;
    }
...
void TetVesicleVesRaft::_useVesR2V() {
    ...
    for (auto const& ves_linkspecpair_r2v: vesLinkSpecPairR2V_Vec) {
        ...
        auto& linkspec1_def = statedef().linkspecdef(linkspec1_global_id);
        auto& linkspec2_def = statedef().linkspecdef(linkspec2_global_id);
        ...
        auto linkspec1 = new LinkSpec(
            &linkspec1_def, linkspec1_individual_id, ves1, tet_gidx, linkspec1_pos_rel);
        ...
        auto linkspec2 = new LinkSpec(
            &linkspec2_def, linkspec2_individual_id, ves2, tet_gidx, linkspec2_pos_rel);
    ...
    for (auto const& ves_linkspec_r2v: vesLinkSpecR2V_Vec) {
        ...
        auto& linkspec_def = statedef().linkspecdef(linkspec_global_id);

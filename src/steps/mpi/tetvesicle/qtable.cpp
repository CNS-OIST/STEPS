...
Qtable::Qtable(uint size, double tau, const rng::RNGptr& r)
    : pTablesize(size)
    , pTau(tau)
...
void Qtable::setup() {
    ...
    uint nvals = 10001;
    double x[10001];      // to avoid compile warnings with double x[nvals]
    double y_cdf[10001];  // to avoid compile warnings with double y_cdf[nvals]
...
void Qtable::reinit(uint size, double tau) {
    pTablesize = size;
    pTau = tau;
...
double Qtable::getPhi() {
    constexpr uint maxRetries = 10000;

...
void TriRDEF::setupKProcs(TetVesicleRDEF* tex, bool efield) {
    ...
    if (hostRank == myRank) {
        ...
        for (auto i: solver::raftgen_local_id::range(nrgens)) {
            solver::RaftGendef* raftgendef = patchdef()->raftgendef(i);
        ...
        for (auto i: solver::raftsreac_global_id::range(nrsreacs)) {
            solver::RaftSReacdef* raftsreacdef = patchdef()->statedef()->raftsreacdef(i);
...
bool TriRDEF::KProcDepSpecTet(uint kp_lidx,
    ...
    if (remain < pPatchdef->countSReacs()) {
        solver::SReacdef* srdef = patchdef()->sreacdef(solver::sreac_local_id(remain));
    ...
    if (remain < pPatchdef->statedef()->countRaftSReacs()) {
        solver::RaftSReacdef* rsrdef = patchdef()->statedef()->raftsreacdef(
    ...
    if (hasEfield) {
        ...
        if (remain < pPatchdef->countVDepSReacs()) {
            solver::VDepSReacdef* vdsrdef = patchdef()->vdepsreacdef(
                solver::vdepsreac_local_id(remain));
            if (kp_container == iTet()) {
        ...
        if (remain < pPatchdef->countGHKcurrs()) {
            solver::GHKcurrdef* ghkdef = patchdef()->ghkcurrdef(solver::ghkcurr_local_id(remain));
            if (kp_container == iTet()) {
...
bool TriRDEF::KProcDepSpecTri(uint kp_lidx,
    ...
    // if kp is surface diff
    if (remain < pPatchdef->countSurfDiffs()) {
        solver::SurfDiffdef* sddef = patchdef()->surfdiffdef(solver::surfdiff_local_id(remain));
    ...
    // if kproc is raft gen
    if (remain < pPatchdef->countRaftGens()) {
        solver::RaftGendef* rgdef = patchdef()->raftgendef(solver::raftgen_local_id(remain));

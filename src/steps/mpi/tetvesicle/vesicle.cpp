...
Vesicle::Vesicle(solver::Vesicledef* vesdef,
    ...
    for (auto const& ls_id: linkspecs) {
        auto& lspecdef = def()->statedef().linkspecdef(ls_id);
        // Constructor does the restore
...
bool Vesicle::setPosition(const overlap::Vector& new_pos,
    ...
    if (check_comps) {
        for (auto const& tet: tets_overlap_temp) {
            if (tet.first == pCentral_tet) {
                continue;
            }
            auto tet_comp = solver()->tet_(tet.first)->getCompVesRaft();
            if (tet_comp != pComp_central and
                !pComp_central->checkVesiclePermittedComp(idx(), tet_comp)) {
                CLOG(WARNING, "general_log")
                    << "After setting new position, Vesicle" << getUniqueIndex()
                    << "overlaps two compartments between which diffusion cannot proceed.\n ";
                return true;
            }
        }
    }
...
void Vesicle::redInnerSpecCounts(const std::map<solver::spec_global_id, uint>& specs) {
    for (auto const& spec: specs) {
        AssertLog(pInnerSpecCount[spec.first] >= spec.second);
        pInnerSpecCount[spec.first] -= spec.second;
    }
}

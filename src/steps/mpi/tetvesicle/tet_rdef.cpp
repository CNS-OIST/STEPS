...
void TetRDEF::setupKProcs(TetVesicleRDEF* tex) {
    ...
    if (hostRank == myRank) {
        ...
        for (auto i: solver::reac_local_id::range(nreacs)) {
            solver::Reacdef* rdef = compdef()->reacdef(i);
        ...
        for (auto i: solver::diff_local_id::range(ndiffs)) {
            solver::Diffdef* ddef = compdef()->diffdef(i);
        ...
        for (auto i: solver::vesbind_local_id::range(vesbinds)) {
            solver::VesBinddef* vbdef = compdef()->vesbinddef(i);
        ...
        for (auto i: solver::vessreac_global_id::range(nvsreacs)) {
            solver::VesSReacdef* vsrdef = compdef()->statedef()->vessreacdef(i);
        ...
        for (auto i: solver::exocytosis_global_id::range(nexos)) {
            solver::Exocytosisdef* exodef = compdef()->statedef()->exocytosisdef(i);
...
bool TetRDEF::KProcDepSpecTetVesSurface(uint kp_lidx,
    ...
    if (remain < compdef()->countVesBinds()) {
        auto const vbdef = compdef()->vesbinddef(solver::vesbind_local_id(remain));
    ...
    if (remain < compdef()->statedef()->countVesSReacs()) {
        solver::VesSReacdef* vsrdef = compdef()->statedef()->vessreacdef(
            solver::vessreac_global_id(remain));
    ...
    if (remain < compdef()->statedef()->countExocytosis()) {
        solver::Exocytosisdef* exodef = compdef()->statedef()->exocytosisdef(
...
bool TetRDEF::KProcDepSpecTet(uint kp_lidx,
    ...
    uint remain = kp_lidx;
    if (remain < compdef()->countReacs()) {
        solver::Reacdef* rdef = compdef()->reacdef(solver::reac_local_id(remain));

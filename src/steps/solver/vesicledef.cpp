...
Vesicledef::Vesicledef(Statedef& sd, vesicle_global_id idx, model::Vesicle& v)
    ...
    , pLinkSpecsN_global(sd.countLinkSpecs()) {
...
void Vesicledef::checkpoint(std::fstream& cp_file) const {
...
void Vesicledef::reset() {
    ...
    for (auto i: exocytosis_local_id::range(countExocytosis())) {
        pExocytosisKcst[i] = exocytosisdef(i).kcst();
    }
}
...
void Vesicledef::setup_references() {
    ...
    for (auto const& s: pVssys) {
        ...
        for (auto const& [_, vessr]: vessreacs) {
            vessreac_global_id gidx = pStatedef.getVesSReacIdx(*vessr);
        ...
        for (auto const& [_, exo]: sexos) {
            exocytosis_global_id gidx = pStatedef.getExocytosisIdx(*exo);
        ...
        for (auto const& [_, vessd]: vessdiffs) {
            vessdiff_global_id gidx = pStatedef.getVesSDiffIdx(*vessd);
    ...
    for (auto sr: vessreac_global_id::range(ngvsreacs)) {
        ...
        const VesSReacdef& vessrdef = pStatedef.vessreacdef(sr);
    ...
    // Nothing to do for exocytosis because it's very difficult to predict
    // which species will be involved

    for (auto vsd: vessdiff_global_id::range(ngvsdiffs)) {
        if (vessurfdiffG2L(vsd).unknown()) {
            continue;
        }
        const VesSDiffdef& vessddef = pStatedef.vessdiffdef(vsd);
        for (auto s: spec_global_id::range(ngspecs)) {
...
void Vesicledef::setup_indices() {
    ...
    if (countVesSReacs() != 0) {
        ...
        for (auto ri: vessreac_local_id::range(countVesSReacs())) {
            const VesSReacdef& vessrdef = vessreacdef(ri);
    ...
    if (countVesSurfDiffs() != 0) {
        ...
        for (auto di: vessdiff_local_id::range(countVesSurfDiffs())) {
            const VesSDiffdef& vessddef = vessurfdiffdef(di);

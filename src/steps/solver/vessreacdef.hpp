...
class VesSReacdef {
  public:
    ...
    /// \param sr Reference to the VesSReac object.
    VesSReacdef(Statedef& sd, vessreac_global_id idx, model::VesSReac& sr);

    VesSReacdef(const VesSReacdef&) = delete;
    VesSReacdef& operator=(const VesSReacdef&) = delete;
  ...
  private:
    const vessreac_global_id pIdx;
    // Have to store this to access the original kcsts to allow for reset
    const model::VesSReac& pVessreac;
    const std::string pName;
    const uint pOrder;
    double pKcst;
    const double pDefaultKcst;
    const unsigned int pCountSpecs;
    const unsigned int pCountLinkSpecs;

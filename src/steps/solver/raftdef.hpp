...
class Raftdef {
  public:
    ...
    /// \param r Reference to the associated Raft object.
    Raftdef(Statedef& sd, raft_global_id idx, model::Raft& r);

    Raftdef(const Raftdef&) = delete;
    Raftdef& operator=(const Raftdef&) = delete;
  ...
  public:
    ...
    /// Return a reference to reaction definition object (type RaftSReacdef)
  ...
  public:
    ...
    /// Return a reference to reaction definition object (type RaftEndocytosis)
    /// specified by local index.
    ...
    /// Return a reference to reaction definition object (type RaftDisdef)
    /// specified by local index.
    RaftDisdef& raftdisdef(raftdis_local_id lidx) const;

  private:
    const Statedef& pStatedef;
    const raft_global_id pIdx;
    ...
    const util::flat_set<std::string> pRssys;

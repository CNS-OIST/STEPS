...
VDepSReacdef::VDepSReacdef(Statedef& sd, vdepsreac_global_id idx, model::VDepSReac& vdsr)
    ...
    if (vdsr.getOrder() == 0) {
        ...
        os << "Model contains zero-order voltage-dependent surface reaction: '" << pName
           << "'. Zero-order voltage-dependent surface reactions are not permitted. ";
        ArgErrLog(os.str());
    }

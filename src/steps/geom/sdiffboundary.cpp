...
SDiffBoundary::SDiffBoundary(std::string id, Tetmesh *container,
  ...
  for (uint bar : bars) {
    ...
    triangle_id_t innertriidx(std::nullopt);
    triangle_id_t outertriidx(std::nullopt);
    ...
    ArgErrLogIf(innertriidx.unknown(), "Patch" + patches[0]->getID() +
                                                " is not connected to bar " +
                                                std::to_string(bar));

    ArgErrLogIf(outertriidx.unknown(), "Patch" + patches[1]->getID() +
                                                " is not connected to bar " +
                                                std::to_string(bar));
...
std::vector<bool>
SDiffBoundary::isBarInside(const std::vector<index_t> &bars) const {

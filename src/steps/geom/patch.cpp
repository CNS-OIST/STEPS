...
Patch::Patch(std::string id, Geom* container, Comp* icomp, Comp* ocomp, double area)
    ...
    ArgErrLogIf(pContainer == nullptr, "No container provided to Patch initializer function.\n");
...
std::vector<model::Spec*> Patch::getAllSpecs(const model::Model* model) const {
    std::set<model::Spec*> specs;

    for (const auto& id: pSurfsys) {
        model::Surfsys* surfsys = model->getSurfsys(id);
        // std::vector<model::Spec *> specs = surfsys->getAllSpecs();
...
std::vector<model::SReac*> Patch::getAllSReacs(const model::Model* model) const {
    std::set<model::SReac*> pSReacs;
    for (const auto& id: pSurfsys) {
        model::Surfsys* surfsys = model->getSurfsys(id);
        std::vector<model::SReac*> sreacs = surfsys->getAllSReacs();

...
class Comp {
  public:
    ...
    std::vector<model::Reac*> getAllReacs(const model::Model* model) const;

    /// Return all Diff in the compartment giving a model.
    std::vector<model::Diff*> getAllDiffs(const model::Model* model) const;
    ...
    inline const std::set<wm::Patch*>& getIPatches() const noexcept {
        return pIPatches;
    }
    ...
    inline const std::set<wm::Patch*>& getOPatches() const noexcept {
        return pOPatches;
    }
  ...
  private:
    ...
    std::set<wm::Patch*> pIPatches;
    std::set<wm::Patch*> pOPatches;

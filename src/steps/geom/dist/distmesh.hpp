...
class DistMesh : public steps::wm::Geom {
  public:
    ...
    static constexpr int dim() noexcept { return mesh_dimensions(); }

    static osh::Mesh load_mesh(osh::Library &library, const std::string &path);

    inline std::string getBackend() const { return "Omega_h"; }
   ...
   /**
    ...
    getTetTetNeighb(mesh::tetrahedron_global_id_t tet_index) const;
    ...
    getTetTetNeighb(mesh::tetrahedron_local_id_t tet_index) const;
    ...
    getTetTriNeighb(mesh::tetrahedron_global_id_t tet_index);
    ...
    getTetTriNeighb(mesh::tetrahedron_local_id_t tet_index);
     ...
     */
    std::vector<double>
    getTetBarycenter(mesh::tetrahedron_global_id_t tet_index) const;
     ...
     */
    std::vector<double>
    getTetBarycenter(mesh::tetrahedron_local_id_t tet_index) const;
     ...
     */
    std::vector<mesh::tetrahedron_local_id_t>
    getLocalTetIndices(bool owned = true);
    ...
    getTet_(mesh::tetrahedron_global_id_t tet_index);
    ...
    getTet_(mesh::tetrahedron_local_id_t tet_index);
   ...
   /**
     ...
     */
    std::vector<mesh::tetrahedron_global_id_t>
    getTriTetNeighb(mesh::triangle_global_id_t tri_index);
     ...
     */
    std::vector<mesh::tetrahedron_local_id_t>
    getTriTetNeighb(mesh::triangle_local_id_t tri_index);
     ...
     */
    std::vector<double>
    getTriBarycenter(mesh::triangle_global_id_t tri_index) const;
     ...
     */
    std::vector<double>
    getTriBarycenter(mesh::triangle_local_id_t tri_index) const;
     ...
     * \return Vector of local vertex indices.
     */
    std::vector<mesh::vertex_local_id_t> getLocalVertIndices(bool owned = true);
     ...
     */
    osh::GO getGlobalIndex(mesh::tetrahedron_local_id_t index) const;
    ...
    getLocalIndex(mesh::tetrahedron_global_id_t index, bool owned = true) const;
     ...
     * \return Global index of the mesh element.
     */
    osh::GO getGlobalIndex(mesh::triangle_local_id_t index) const;
     ...
     * \return Local index of the mesh element.
     */
    mesh::triangle_local_id_t getLocalIndex(mesh::triangle_global_id_t index,
                                            bool owned = true) const;

    osh::GO getGlobalIndex(mesh::vertex_local_id_t index) const;
    mesh::vertex_local_id_t getLocalIndex(mesh::vertex_global_id_t index,
                                          bool owned = true) const;
    ...
    template <typename T> inline T get_MPI_max(const osh::Read<T> &a) const {
      return osh::get_max(mesh_.comm(), a);
    }

    /// Min over the various MPI processes
    template <typename T> inline T get_MPI_min(const osh::Read<T> &a) const {
    ...
    inline const Measure &getMeasure() const noexcept { return *measure_; }
    ...
    measure(const model::region_id &region);
    ...
    const auto &getClassSets() const noexcept { return mesh_.class_sets; }

    mesh::compartment_id
    getCompID(const model::compartment_id &compartment) noexcept;

    mesh::patch_id getPatchID(const model::patch_id &patch) noexcept;

    model::compartment_id getCompartment(mesh::tetrahedron_id_t element) const
        noexcept;

    mesh::compartment_id
    getCompartmentMeshID(mesh::tetrahedron_id_t element) const noexcept {
      return elem2compid_[element.get()];
    }
    /**
    ...
    getOwnedEntities(const model::compartment_id &compartment);
     ...
     */
    std::vector<mesh::tetrahedron_global_id_t>
    getTaggedTetrahedrons(const model::compartment_id &comp);
     ...
     */
    std::vector<mesh::tetrahedron_local_id_t>
    getTaggedLocalTetrahedrons(const model::compartment_id &comp, bool owned=true);
     ...
     */
    std::vector<mesh::triangle_global_id_t>
    getTaggedTriangles(const model::patch_id &patch);
     ...
     */
    std::vector<mesh::triangle_local_id_t>
    getTaggedLocalTriangles(const model::patch_id &patch, bool owned = true);
     ...
     */
    std::vector<mesh::vertex_global_id_t>
    getTaggedVertices(const model::vertgroup_id &verts);
     ...
     */
    std::vector<mesh::vertex_local_id_t>
    getTaggedLocalVertices(const model::vertgroup_id &verts, bool owned = true);
    ...
    const std::unordered_map<model::compartment_id, model::compartment_label> &
    compartment_labels() const noexcept {
     ...
     */
    inline container::species_id
    convertSpeciesID(mesh::triangle_id_t triangle,
      ...
      return (db.msh_comp1 == from_comp_id)
                 ? diffusion_boundaries_
                       [*diffusion_boundary_ids_[triangle.get()]]
                           .conv_12[spec_id.get()]
                 : diffusion_boundaries_
                       [*diffusion_boundary_ids_[triangle.get()]]
                           .conv_21[spec_id.get()];
    ...
    inline bool isActiveDiffusionBoundary(mesh::triangle_id_t triangle,
      ...
      if (diffusion_boundary_ids_[triangle.get()]) {
        return (diffusion_boundaries_[*diffusion_boundary_ids_[triangle.get()]]
                    .msh_comp1 == comp_id)
                   ? diffusion_boundaries_
                         [*diffusion_boundary_ids_[triangle.get()]]
     ...
     */
    size_t
    getDiffusionBoundaryIndex(const mesh::diffusion_boundary_name &name) const {
      auto it = diff_bound_name_2_index_.find(name);
      if (it != diff_bound_name_2_index_.end()) {
        return it->second;
      } else {
        throw std::invalid_argument(std::string("Unknown diffusion boundary ") +
                                    name);
      }
    }
  ...
  private:
    ...
    void fill_triInfo(const Omega_h::Reals &coords,
    ...
    osh::LOs getEntitiesImpl(const util::strong_string<Tag> &region,
                             bool owned);
    ...
    getClassPairs(const model::vertgroup_id &verts) const;
     ...
     */
    std::vector<osh::ClassPair>
    getClassPairs(const model::compartment_id &compartment) const;
     ...
     */
    std::vector<osh::ClassPair>
    getClassPairs(const model::patch_id &patch) const;
    ...
    std::unordered_map<mesh::tetrahedron_global_id_t,
                       mesh::tetrahedron_local_id_t>
        elemGlobal2Local;
    ...
    std::unordered_map<mesh::triangle_global_id_t, mesh::triangle_local_id_t>
        boundGlobal2Local;
    ...
    std::unordered_map<mesh::compartment_id, mesh::tetrahedron_ids>
        compid2elems_;
    ...
    std::unordered_map<model::compartment_label, model::compartment_id>
        compLabelToId;
    std::unordered_map<model::compartment_id, model::compartment_label>
        compIdtoLabel;

...
const std::vector<mesh::triangle_local_id_t>& DistPatch::getLocalTriIndices(bool owned) const {
    if (owned) {
...
void DistPatch::_computeBBox() {
    ...
    for (const auto tri: ownedTriLocalIndices) {
        ...
        const auto tri2x = osh::gather_vectors<3, mesh_dimensions()>(meshRef.coords(), tri2verts);

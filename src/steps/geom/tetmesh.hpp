...
class Tetmesh: public wm::Geom {
  public:
    ...
    /// \param sdiffb Reference to the associated surface diffusion boundary.
    ...
    /// \return Vector of the triangle neighbors.
    ///
    std::vector<index_t> getTriTriNeighbs(triangle_global_id tidx) const;
    ...
    /// Return a weighted mesh partition
    /// \param seed Seed of the random partitioning.
    /// \param nparts Number of partitions.
    /// \param weights Weights for each tetrahedron.
    ///
    /// \return A vector where each position contains the partition index of
    /// the corresponding tetrahedron.
    std::vector<int> getWeightedPartition(int seed, uint nparts, std::vector<int> weights);
  ...
  private:
    ...
    std::map<std::string, Memb*> pMembs;

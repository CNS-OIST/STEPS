/*
 ...
 #    Copyright (C) 2007-2017 Okinawa Institute of Science and Technology,
 Japan. #    Copyright (C) 2003-2006 University of Antwerp, Belgium.
 #
...
DiffBoundary::DiffBoundary(std::string id, Tetmesh *container,
    ...
    : pID(std::move(id)), pTetmesh(container) {
  ArgErrLogIf(pTetmesh == nullptr,
              "No mesh provided to Diffusion Boundary initializer function.");
  ArgErrLogIf(
      tris.empty(),
      "No triangles provided to Diffusion Boundary initializer function.");
  ...
  for (auto tri : tris) {
    ...
    ArgErrLogIf(tri > maxidx,
                "Invalid triangle index " + std::to_string(tri) + ".");
...
std::vector<bool>
DiffBoundary::isTriInside(std::vector<index_t> const &tris) const {

...
Memb::Memb(std::string id,
    ...
    ArgErrLogIf(patches.empty(), "No Patches provided to Membrane initializer function.");
    ArgErrLogIf(pOpt_method != 1 && pOpt_method != 2,
                "Unknown optimization method. Choices are 1 or 2.");
    ArgErrLogIf(pSearch_percent > 100.0, "Search percentage is greater than 100.");
    ArgErrLogIf(pSearch_percent <= 0.0, "Search percentage must be greater than 0.");

    std::set<TmComp*> comps_set;
    for (auto p: patches) {
        ...
        comps_set.insert(comp);
    }

    // Additional compartments for conduction volume
    for (auto comp: compartments) {
        ArgErrLogIf(comps_set.find(comp) != comps_set.end(),
                    "Additional compartment " + comp->getID() +
                        " is already part of the conduction volume.");
        comps_set.insert(comp);
    }

    // Emit warnings if compartments in conduction volumes are separated by patches
    std::set<tetrahedron_global_id> tets_set;
    for (auto comp: comps_set) {
        for (auto op: comp->getOPatches()) {
            auto ocomp = dynamic_cast<TmComp*>(op->getOComp());
            if (comps_set.find(ocomp) != comps_set.end()) {
                CLOG(WARNING, "general_log")
                    << "WARNING: Compartments" << comp->getID() << "and" << ocomp->getID()
                    << "are both part of the conduction volume for membrane" << pID
                    << "but they are also separated by patch" << op->getID()
                    << ". Note that this patch will not block internal currents.";
            }
        }
    ...
    for (auto tet: pTet_indices) {
        ...
        verts_set.insert(tet_verts.begin(), tet_verts.end());
    }
...
std::vector<bool> Memb::isTriInside(const std::vector<index_t>& tri) const {
    return util::map_membership(tri, pTri_indices);
}
...
void Memb::verifyMemb() {
    ...
    for (auto const& tri: pTri_indices) {
        ...
        // are
        auto const& ttn = pTetmesh.getTriTriNeighbs(tri);
        std::set<index_t> tritrineighbs(ttn.begin(), ttn.end());
        ...
        if (neighbours > 3) {
            CLOG(INFO, "general_log")
                << "WARNING: Triangle " << tri << " has " << neighbours << " neighbours.\n";
        }
    }
    ...
    for (auto const& tri: pTri_indices) {
        ...
        auto tritetneighbs = pTetmesh._getTriTetNeighb(tri);
    ...
    if (open()) {
        std::vector<triangle_global_id> trivirttemp;
        ...
        for (auto const& tet: pTet_indices) {
            ...
            for (uint j = 0; j < 4; ++j) {
                if (!gotneighb[j]) {
                    ...
                    if (!membtri) {
                        trivirttemp.emplace_back(pTetmesh.getTetTriNeighb(tet)[j]);
                    }
                }
            }
        }
        ...
        std::set<triangle_global_id> trivirt_set;

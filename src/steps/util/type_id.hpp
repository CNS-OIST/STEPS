 ...
 */
template <class Enum, int EnumBits, class Id> class CompactTypeId {
public:
  ...
  static_assert(std::is_integral<id_t>::value,
                "Only integral types are supported for id_t");

  constexpr explicit CompactTypeId(data_t data) : data_(data) {}

  constexpr CompactTypeId(Enum type, id_t id) : data_(encode(type, id)) {
    assert(fits(type));
    assert(fits(id));
  }
  ...
  constexpr data_t data() const noexcept { return data_; }
  ...
  static constexpr data_t encode(Enum type, id_t id) {
    return static_cast<data_t>(type) << num_right_shifts_for_type() |
           static_cast<data_t>(id);
  }

  /// \return the number of bits allocated to encode the type
  static constexpr int num_bits_for_type() { return EnumBits; }
...
private:
  ...
  static constexpr data_t id_mask() { return ~data_t{} >> num_bits_for_type(); }

...
template <typename T> using Vec2 = std::vector<Vec1<T>>;
...
template <typename T> void iterMeanVec3(const Vec3<T> &data, Vec2<T> &result) {
...
template <typename T> void iterSTDVec3(const Vec3<T> &data, Vec2<T> &result) {
  auto niters = data.size();
  auto ntpnts = data[0].size();
  auto nelems = data[0][0].size();
  result.assign(ntpnts, Vec1<T>(nelems, 0.0));

...
osh::Vector<Dim> barycenter(const osh::Matrix<Dim, Dim + 1> &points) {
  const auto &point =
      std::accumulate(points.begin(), points.end(), osh::Vector<Dim>(),
 ...
 */
template <osh::Int Dim>
static osh::Few<osh::LO, Dim + 1>
gather_neighbours(osh::Adj const &boundary2elems,
                  osh::LOs const &elems2boundary, osh::LO elem) {
  osh::Few<osh::LO, Dim + 1> neighbours_indexes(
      steps::util::initializer_list<Dim + 1>(-1));
  ...
  for (auto boundary = 0; boundary < elem2boundaries.size(); ++boundary) {
    ...
    if (b2be[boundary_index + 1] - offset > 1) {
      ...
      neighbours_indexes[boundary] =
          be2e[offset + (be2e[offset] == elem ? 1 : 0)];
    }
  }
...
template <osh::Int N>
bool operator<(osh::Vector<N> const &a,
               osh::Vector<N> const &b) OMEGA_H_NOEXCEPT {
  for (auto i = 0; i < N; ++i) {
    if (a[i] >= b[i]) {
      return false;
    }
...
osh::LO point_to_elem(osh::Mesh &mesh, osh::Vector<Dim> point) {
  ...
  for (auto elem = 0; elem < mesh.nelems(); ++elem) {
    ...
    const auto &elem_j2x =
        osh::gather_vectors<Dim + 1, Dim>(coords, elem_j2verts);
    ...
    const auto coeff_sum = std::accumulate(
        coeff.begin(), coeff.end(), osh::Real{0}, std::plus<osh::Real>());
    const auto all_coeff_positive =
        std::accumulate(coeff.begin(), coeff.end(), true,
...
template <bool Overflow> struct PETScCast;
...
template <> struct PETScCast<false> {
...
struct PETScGetValues {
  PetscErrorCode operator()(Vec x, PETSC_INT ni, const osh::GOs &ix,
                            PetscScalar y[]) {
  ...
  const PETSC_INT *
  operator()(const petsc_osh_element_array& array) const noexcept {
    return array.data();
  }
...
struct PETScGetValues<PETSC_INT,
                      typename std::enable_if<std::is_same<
                          typename std::add_pointer<PETSC_INT>::type,
...
struct PETScGetValues<
    ...
    typename std::enable_if<
        std::numeric_limits<PETSC_INT>::max() ==
            std::numeric_limits<osh::GO>::max() &&
        !std::is_same<typename std::add_pointer<PETSC_INT>::type,
                      std::add_pointer<osh::GO>::type>::value>::type> {
  PetscErrorCode operator()(Vec x, PETSC_INT ni, const osh::GOs &ix,
                            PetscScalar y[]) const {
    return VecGetValues(x, ni, reinterpret_cast<const PETSC_INT *>(ix.data()),
                        y);
  }

  PetscErrorCode operator()(Vec x, osh::GO ix, PetscScalar &y) const {
    return VecGetValues(x, PETSC_INT(1),
                        reinterpret_cast<const PETSC_INT *>(&ix), &y);
  }
...
inline PetscErrorCode petsc_get_values(Vec x, PetscInt ni, const osh::GOs &ix,
                                       PetscScalar y[]) {
...
inline const PetscInt *
petsc_pointer(const petsc_osh_element_array &array) noexcept {

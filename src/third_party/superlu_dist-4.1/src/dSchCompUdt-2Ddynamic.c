        ...
        for (j = jj0; j < nub; ++j) {
            ...
            for (jj = iukp; jj < iukp+nsupc; ++jj) {
                ...
                if ( segsize ) {
                    ...
                    if (ldu + (tempu - bigU) > bigu_size) {
                        /* fprintf(stderr,"oob: j=%d; jj=%d; iukp=%d; nsupc=%d; klst=%d; tempu-bigU=%td; ldu=%d; segsize=%d; lead_zero=%d; bigu_size=%zu\n",(int)j,(int)jj,(int)iukp,(int)nsupc,(int)(klst),(ptrdiff_t)(tempu-bigU),(int)ldu,(int)segsize,(int)lead_zero,(size_t)(bigu_size)); */
                        ABORT("Buffer overrun on bigU");
                    }

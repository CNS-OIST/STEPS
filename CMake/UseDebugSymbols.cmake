...
if(CXX_COMPAT_GCC)
  ...
  # add debug symbols to *all* targets, regardless. there WILL come a time when you need to find a
  # bug which only manifests itself in a release target on a production system!
  if(_dbg_flag)
    message(STATUS "Generating debug symbols: ${_dbg_flag}")
    add_options(ALL_LANGUAGES ALL_BUILDS "${_dbg_flag}")
  endif(_dbg_flag)

  # extracting the debug info is done by a separate utility in the GNU toolchain. check that this is
  # actually installed.
  ...
  if(APPLE)
    ...
    find_program(OBJCOPY objcopy ${CYGWIN_INSTALL_PATH}/bin /var/empty/bin /var/empty/local/bin)
  endif(APPLE)
...
# command to separate the debug information from the executable into its own file; this must be
# called for each target; optionally takes the name of a variable to receive the list of .debug
# files
function(strip_debug_symbols targets)
  if(CXX_COMPAT_GCC AND OBJCOPY)
    foreach(target IN LISTS targets)
      # libraries must retain the symbols in order to link to them, but everything can be stripped
      # in an executable
      ...
      # add_custom_command doesn't support generator expressions in the working_directory argument
      # (sic; that's what you get when you do ad hoc programming all the time), so we need to
      # extract the location up front (the location on the other hand should not be used for
      # libraries as it does not include the soversion -- sic again)
      ...
      if(APPLE)
        ...
        add_custom_command(
          ...
          COMMAND ${OBJCOPY} ARGS --add-gnu-debuglink=${_target_file_name}${_debug_ext}
                  ${_target_file}
          VERBATIM)
      endif(APPLE)

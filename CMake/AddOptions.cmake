...
function(add_options langs builds)
  ...
  foreach(lang IN LISTS langs)
    ...
    foreach(build IN LISTS builds)
      ...
      # if we want everything in the "global" flag, then simply ignore the build type here and go
      # add everything to that one
      if(CMAKE_NOT_USING_CONFIG_FLAGS)
        set(_bld "")
      endif()
      foreach(_opt IN LISTS ARGN)
        set(_var "CMAKE_${lang}_FLAGS${_bld}")
        # message (STATUS "Adding \"${_opt}\" to \${${_var}}") remove it first
...
# clear default options as a proxy for not using any default options at all. there is one *huge*
# problem with this: CMake runs the platform initialization before executing any line at all in the
# project and there seems to be no way to disable that behaviour, so we cannot really distinguish
# between a platform default and something that the user has passed on the command line. the best
# thing we can do is to all user- defined setting if they are something other than the platform
# default.

...
function(add_python_tests name path prefix)
  ...
  foreach(TEST_PATH ${STR_TESTS_PATHS})
    ...
    elseif(${prefix} STREQUAL ${PARALLEL_PYTEST_PREFIX})
      add_test(
        NAME "${name}.${TEST_NAME}"
        COMMAND ${CMAKE_COMMAND} -E env ${run_env_vars} ${MPIRUN} -n ${NB_PROCS} ${CMAKE_COMMAND} -E
                env ${run_env_vars} ${PYTHON_TEST_EXECUTABLE} -m unittest ${TEST_PATH}
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR})
      set_tests_properties("${name}.${TEST_NAME}" PROPERTIES PROCESSORS ${NB_PROCS})
    elseif(${prefix} STREQUAL ${DISTRIBUTED_PYTEST_PREFIX})
      add_test(
        NAME "${name}.${TEST_NAME}"
        COMMAND ${CMAKE_COMMAND} -E env ${run_env_vars} ${MPIRUN} -n ${NB_PROCS}

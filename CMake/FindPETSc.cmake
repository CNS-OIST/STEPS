...
#  PETSC_LINK_LIBRARIES    - Link these to use PETSc
...
if(NOT PETSc_FIND_COMPONENTS)
  ...
  # Right now, this is designed for compatability with the --with-clanguage option, so only allow
  # one item in the components list.
  ...
  # This is a stub for allowing multiple components should that time ever come. Perhaps to also test
  # Fortran bindings?
...
function(petsc_get_version)
  if(EXISTS "${PETSC_DIR}/include/petscversion.h")
    ...
    foreach(line ${vstrings})
      string(REGEX REPLACE " +" ";" fields ${line}) # break line into three fields (the first is
                                                    # always "#define")
    ...
    message(
      SEND_ERROR "PETSC_DIR can not be used, ${PETSC_DIR}/include/petscversion.h does not exist")
  endif()
endfunction()
...
# Determine whether the PETSc layout is old-style (through 2.3.3) or new-style (>= 3.0.0)
...
if(petsc_conf_rules
  ...
  # Put variables into environment since they are needed to get configuration (petscvariables) in
  # the PETSc makefile
...
")
  ...
  macro(PETSC_GET_VARIABLE name var)
    ...
    execute_process(
      COMMAND ${MAKE_EXECUTABLE} --no-print-directory -f ${petsc_config_makefile} show
              VARIABLE=${name}
      OUTPUT_VARIABLE ${var}
  ...
  # on windows we need to make sure we're linking against the right runtime library
  if(WIN32)
 ...
 See http://www.cmake.org/Wiki/CMake_FAQ#How_can_I_build_my_MSVC_application_with_a_static_runtime.3F"
  ...
  macro(PETSC_FIND_LIBRARY suffix name)
    set(PETSC_LIBRARY_${suffix}
        "NOTFOUND"
        CACHE INTERNAL "Cleared" FORCE) # Clear any stale value, if we got here, we need to find it
                                        # again
    if(WIN32)
  ...
  if(PETSC_LIBRARY_VEC)
    petsc_find_library(SYS "petscsys;petsc") # libpetscsys is called libpetsc prior to 3.1 (when
    ...
    # Debian 9/Ubuntu 16.04 uses _real and _complex extensions when using libraries in
    # /usr/lib/petsc.
...
")
    multipass_source_runs("${includes}" "${libraries}" "${_PETSC_TEST_SOURCE}" ${runs}
  ...
  # We do an out-of-source build so __FILE__ will be an absolute path, hence __INSDIR__ is
  # superfluous
  ...
  # Note that we have forced values for all these choices.  If you change these, you are telling the
  # system to trust you that they work.  It is likely that you will end up with a broken build.
  mark_as_advanced(PETSC_INCLUDE_DIRS PETSC_LINK_LIBRARIES PETSC_COMPILER PETSC_DEFINITIONS
...
find_package_handle_standard_args(
  PETSc
  REQUIRED_VARS PETSC_INCLUDE_DIRS PETSC_LINK_LIBRARIES PETSC_EXECUTABLE_RUNS

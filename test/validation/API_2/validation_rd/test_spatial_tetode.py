...
# Spatial chemical reactions. 

# AIMS: to verify STEPS spatial deterministic solver 'TetODE' computes 
# reaction rates correctly when many different chemical species and 
# reactions are present in the model. 

# This model combines all validations, as described at: 
...
# Tiny tolerances are allowed to account for small diffusive effects
...
class TestRDSpatialTetODE(unittest.TestCase):

    def test_spatial_tetODE(self):
        ...
        mesh = TetMesh.LoadAbaqus('validation_rd/meshes/sphere_rad1_37tets.inp', scale=1e-06)
        VOL = mesh.Vol
        with mesh:
    ...
    ##################### First order irreversible #########################
        ...
        "Reaction - First order, irreversible (TetODE)"
    ...
    ####################### First order reversible #########################
        ...
        "Reaction - First order, reversible (TetODE)"
    ...
    ####################### Second order irreversible A2 ###################
        ...
        "Reaction - Second order, irreversible, 2A->C (TetODE)"
    ...
    ####################### Third order irreversible A3 ###################
        ...
        "Reaction - Third order, irreversible, 3A->C (TetODE)"
    ...
    ####################### Third order irreversible A3 ###################
        ...
        "Reaction - Third order, irreversible, 2A+B->C (TetODE)"
    ...
    ####################### Second order irreversible AA ###################
        ...
        "Reaction - Second order, irreversible, A0=B0 (TetODE)"
    ...
    ########################################################################
        ...
        "Reaction - Second-order, irreversible, 2D (TetODE)"

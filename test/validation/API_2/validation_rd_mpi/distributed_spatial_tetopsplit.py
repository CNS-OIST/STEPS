...
# AIMS: to verify STEPS distributed stochastic solver 'DistTetOpSplit' computes
...
class TestRDMPISpatialDistTetopsplit(unittest.TestCase):

    def test_spatial_DistTetOpsplit(self):
        self._test_spatial_DistTetOpsplit()

    def test_spatial_DistTetOpsplit_RLeaping(self):
        self._test_spatial_DistTetOpsplit(
            dict(
    ...
    def _test_spatial_DistTetOpsplit(self, solver_kwargs={}):
        ...
        r = ReactionManager()
        with mdl:
            A_foi, A_for, B_for, A_soA2, C_soA2, A_soAA, B_soAA, C_soAA, A_soAB, B_soAB, C_soAB, A_toA3,\
                C_toA3, A_toA2B, B_toA2B, C_toA2B, A_so2d, B_so2d, C_so2d = Species.Create()
            ...
            # TODO: Reactivate when surface diffusion is implemented in DistTetOpSplit
            # with surfsys:
            #     # Second order irreversible 2D
        ...
        mesh = DistMesh("validation_rd_mpi/meshes/sphere_rad1_37tets.msh", scale=1e-06)
        VOL = mesh.Vol
        with mesh:
        ...
        rng = RNG("r123", 512, 200)

        sim = Simulation("DistTetOpSplit", mdl, mesh, rng, **solver_kwargs)
        ...
        for i in range(ntpnts):
            ...
            std = math.pow(
                (
                    N_foi
                    * (math.exp(-KCST_foi * tpnts[i]))
                    * (1 - (math.exp(-KCST_foi * tpnts[i])))
                ),
                0.5,
            )

            self.assertTrue(tol_funcs.tolerable(analy, mean_res_foi[i], tolerance_foi))
            self.assertTrue(tol_funcs.tolerable(std, std_res_foi[i], tolerance_foi))
        ...
        Aeq = (
            COUNT_for
            * (KCST_b_for / KCST_f_for)
            / (1 + (KCST_b_for / KCST_f_for))
            / (VOL * Avogadro * 1e3)
            * 1e6
        )
        ...
        for i in range(ntpnts):
            ...
            lineAB_soAB[i] = (
                math.log(CONCB_soAB / CONCA_soAB) - C_soAB * KCST_soAB * tpnts[i]
            )

            self.assertTrue(
                tol_funcs.tolerable(lnBA_soAB[i], lineAB_soAB[i], tolerance_soAB)
            )
...
def suite():
    all_tests = []
    all_tests.append(
        unittest.TestLoader().loadTestsFromTestCase(TestRDMPISpatialDistTetopsplit)
    )

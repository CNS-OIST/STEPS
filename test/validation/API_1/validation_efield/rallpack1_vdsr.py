...
def build_model(mesh, param):
    ...
    e = 1
    OC_L = smodel.VDepSReac('OC_L', ssys, charge=-e , slhs = [Leak], srhs = [Leak], 
        k = lambda V: (1.0/(e*1.60217663e-19)) * g_leak_sc*(V-param['E_M']), 
        vrange = (-150e-3, 150e-3, 0.01e-3))
...
def run_sim(sim, dt, t_end, vertices, verbose=False):
    ...
    for l in range(N):
        if not l%1000:  print(str(l)+" out of "+str(N), flush=True)
        sim.run(l*dt)

...
# STEPS
    ...
    'EF_solver': 'EF_DV_PETSC', # E-field lin algebra solver
...
def init_sim(model, mesh, seed, param):
    ...
    if param['SSA_solver'] == 'Tetvesicle':
        ...
        rng.initialize(seed)
        sim = ssolver.TetVesicle(model, mesh, rng, ssolver.EF_DV_PETSC)
        sim.reset()
    ...
    memb = sgeom.castToTmPatch(mesh.getPatch('memb'))
    #for t in memb.tris: sim.setTriSpecCount(t, 'Leak', 0) #turn leak channel off
    ...
    sim.setMembRes('membrane', param['R_M']*area_mesh_factor, param['E_M'])

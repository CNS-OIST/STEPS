...
# solver 'Tetvesicle' in the context of the Degradation-Diffusion model
...
class TestKisilevichTetvesicleCP(unittest.TestCase):
    ...
    def setUp(self):
        ...
        sim.checkpoint('./validation_cp_mpi/cp/kisilevich')

    ########################################################################

    def test_kis_n4(self):
        myrank = steps.mpi.rank
        ...
        for i in range (0, NITER):
            sim.restore('./validation_cp_mpi/cp/kisilevich')
            rng.initialize(seed)
            seed += 1

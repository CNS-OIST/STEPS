...
from steps.sim import SimPathInvalidPath, SimPathSolverMissingMethod
...
FILEDIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../geom_test')
...
class TestModelFramework(unittest.TestCase):
    ...
    def _test_API2_SimPathGetSyntax(self, sim, init=True, usingMesh=False, extents=None):
        ...
        if usingMesh:
            ...
            self.assertAlmostEqualWThresh(
                sum(sim.TETS().S1.Count),
                sim.comp1.S1.Count + sim.comp2.S1.Count,
                self.tolerance
            )
            self.assertAlmostEqualWThresh(
            ...
            if sim.solver.__class__.__name__ != 'TetOpSplit':
    ...
    def _test_API2_SimPathSetSyntax(self, sim, usingMesh=False):
        ...
        if usingMesh:
            ...
            self.assertAlmostEqualWThresh(sim.VERT(vert1).IClamp, 1.7852e-7, self.tolerance)
...
class TetTestModelFramework(TestModelFramework):
    ...
    def get_API2_Mdl(self, sas=True):
        ...
        self.c1DCC = CompDepDcst(lambda s: s.Count(nmdl.sus2) * self.CCmult, [nmdl.CC])

        self.rvdep01f = VDepRate(lambda v: 2.0, vrange=self.vrange)
        self.rvdep01b = VDepRate(lambda v: 1.0, vrange=self.vrange)

        with nmdl:
            chanop, chancl = SubUnitState.Create()
            Chan1 = Channel.Create([chanop, chancl])

            nmdl.S3.valence = 1

            with nmdl.ssys, Chan1[...]:
                chancl.s <r['ss1RVDep01']> chanop.s
                r['ss1RVDep01'].K = self.rvdep01f, self.rvdep01b

                Chan1_Ohm_I = OhmicCurr.Create(Chan1[chanop], self.Chan1_G, self.Chan1_rev)
                Chan1_GHK_I = GHKCurr.Create(Chan1[chanop], nmdl.S3, self.Chan1_P, computeflux=True)
    ...
    def get_API1_Mdl(self):
        ...
        chanop = smodel.ChanState('chanop', omdl, Chan1)
        chancl = smodel.ChanState('chancl', omdl, Chan1)
        ss1RVDep01_fwd = smodel.VDepSReac('ss1RVDep01_fwd', omdl.getSurfsys('ssys'), slhs=[chancl], srhs=[chanop], k=self.rvdep01f._func, vrange=self.rvdep01f._vrange)
        ss1RVDep01_bkw = smodel.VDepSReac('ss1RVDep01_bkw', omdl.getSurfsys('ssys'), srhs=[chancl], slhs=[chanop], k=self.rvdep01b._func, vrange=self.rvdep01b._vrange)

        Chan1_Ohm_I = smodel.OhmicCurr('Chan1_Ohm_I', omdl.getSurfsys('ssys'), chanstate=chanop, g=self.Chan1_G, erev=self.Chan1_rev)
        Chan1_GHK_I = smodel.GHKcurr('Chan1_GHK_I', omdl.getSurfsys('ssys'), chanop, omdl.getSpec('S3'), computeflux = True)
    ...
    def get_API2_Geom(self, nmdl):
        ...
        nmesh = TetMesh.Load(os.path.join(FILEDIR, 'meshes', 'cyl_len10_diam1'))

        with nmesh:
            center1 = nmesh.tets[0, 0, nmesh.bbox.max.z / 2]
            self.c1tets = TetList([tet for tet in nmesh.tets if tet.center.z > 0 and tet.idx != center1.idx])
            c2tets = nmesh.tets - self.c1tets - TetList([center1])
            comp1 = Compartment.Create(self.c1tets, nmdl.vsys1)
            comp1bis = Compartment.Create([center1], nmdl.vsys1)
            comp2 = Compartment.Create(c2tets, nmdl.vsys2)
    ...
    def get_API1_Geom(self, omdl):
        ...
        omesh = smeshio.loadMesh(os.path.join(FILEDIR, 'meshes', 'cyl_len10_diam1'))[0]

        n = omesh.countTets()
        center1 = omesh.findTetByPoint([0, 0, omesh.getBoundMax()[2] / 2])
        oc1tets = [i for i in range(n) if omesh.getTetBarycenter(i)[2] > 0 and i != center1]
        oc1btets = [center1]
        oc2tets = [i for i in range(n) if omesh.getTetBarycenter(i)[2] <= 0 and i != center1]

        trioverlap = smeshctrl.findOverlapTris(omesh, oc1tets, oc2tets)
        optris = [i for i in trioverlap if omesh.getTriBarycenter(i)[0] > 0]
        optris2 = [i for i in trioverlap if omesh.getTriBarycenter(i)[0] <= 0]

        comp1 = sgeom.TmComp('comp1', omesh, oc1tets)
        comp1.addVolsys('vsys1')
        comp1bis = sgeom.TmComp('comp1bis', omesh, oc1btets)
        comp1.addVolsys('vsys1')
        comp2 = sgeom.TmComp('comp2', omesh, oc2tets)
        comp2.addVolsys('vsys2')
        patch = sgeom.TmPatch('patch', omesh, optris, comp1, comp2)
        patch.addSurfsys('ssys')
        patch2 = sgeom.TmPatch('patch2', omesh, optris2, comp1, comp2)
        patch2.addSurfsys('ssys')

        omesh.addROI('comp1ROI', sgeom.ELEM_TET, oc1tets)
        omesh.addROI('patchROI', sgeom.ELEM_TRI, optris)
        verts = []
        for i in optris:
    ...
    def init_API2_sim(self, sim):
        ...
        sim.patch.Chan1[self.newMdl.chancl].Count = self.initChan1Cl

...
TEST_F(TetmeshTest,intersectMontecarlo_all3) {
    ...
    auto res = mesh->intersectMontecarlo(p0, p1, steps::UNKNOWN_TET, nsample);
...
TEST_F(TetmeshTest,intersectDeterministic_notFirstNeighbor) {
    ...
    std::vector<double> mesh_verteces  {0.00012257076822803781, 0.0006780064542649207, 0.00042064630907423571,
    ...
    std::vector<index_t> mesh_tets     {0, 1, 2, 3,
                                        0, 2, 1, 4,
                                        0, 5, 2, 4,
                                        0, 5, 6, 2,
                                        6, 0, 7, 5,
    ...
    auto res = mesh2->intersectDeterministic(p0, p1);
    for (const auto& r: res)
        std::cout << r.first << ": " << r.second << std::endl;

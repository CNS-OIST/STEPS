...
void binomial_check(const std::string& str,
    ...
    if ((t == t_test) && (fabs(p - p_test) < 1e-14 * p)) {
        ASSERT_GE(p_value, 1 - level_confidence)
            << "FAILED Goodness of Fit test (chi-squared) with level of confidence "
            << level_confidence << ": p_value is " << p_value
            << " which is not big enough to support the hypothesis of binomial distribution"
            << std::endl;
    } else {
        ASSERT_LE(p_value, level_confidence)
...
void assert_pvalue(const double p_value, const double level_confidence) {
    ...
    ASSERT_GE(p_value, 1. - level_confidence)
        << "Kendall correlation test has p_value = " << p_value
...
TEST(rng, class_create_mt) {
    ...
    ASSERT_THROW(create("mt19937", 0), steps::ArgErr);
...
TEST(rng, class_create_r123) {
    ...
    ASSERT_THROW(create("r123", 0), steps::ArgErr);
    ASSERT_THROW(create("abc", n_sample), steps::ArgErr);
}

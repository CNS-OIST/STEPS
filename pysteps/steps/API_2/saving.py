...
class UnavailableSelectorError(KeyError):
    """
    :meta private:
    """
    def __init__(self, descr):
        super().__init__(f'No result selector have the following description: {descr}')
        self.descr = descr
...
class ResultSelector:
    ...
    def description(self):
        ...
        All result selectors have a default string description generated by STEPS.
...
class CustomResults(ResultSelector):
    ...
    def save(self, values):
        ...
        Usage::
            ...
            cr1 = CustomResults(sim, [dict, list, float])
        ...
        """
        self._checkAddedToSim()
        if self._distrInds is None or len(self._distrInds) > 0:
    ...
    def _distribute(self):
        ...
        if not nsim.MPI._shouldWrite:
            ...
            self._distrInds = list(range(self._fullLen))
            return self, False
    ...
    def _finalize(self):
        ...
        *_descr, endName = self._descr
        for descr in self.simpath._getDescriptions(tuple(_descr)):
            lbls.append('.'.join(descr[1:] + (endName,)))
        ...
        if self._simPathMask is not None:
            ...
            lbls = [lbl for lbl, msk in zip(lbls, self._simPathMask) if msk]
            mtdt = {key:[v for v, msk in zip(lst, self._simPathMask) if msk] for key, lst in mtdt.items()}
        # Add previously defined metadata and labels, if applicable
        if self._distrInds is not None:
            if self._labels is not None:
                lbls = [self._labels[i] for i in self._distrInds]
            for key, vals in self._metaData.items():
                mtdt.setdefault(key, [vals[i] for i in self._distrInds])
        # Set the labels
        self._labels = lbls
        # Set the metadata
        for key, lst in mtdt.items():
...
class _ResultList(ResultSelector):
    ...
    def _distribute(self, updateLblMtdt=True):
        """Distribute the path across MPI ranks if it involves mesh elements of a distributed meshes."""
        if self._distrInds is not None:
            return self, False
        ...
        if updateLblMtdt:
            self._labels = [self._labels[i] for i in self._distrInds]
...
class _ResultCombiner(_ResultList):
    ...
    def _distribute(self):
        ...
        if self._distribIndFunc is not None and 'loc_id' in self.metaData and all(v is not None for v in self.metaData['loc_id']):
            dist, changed = super()._distribute(updateLblMtdt=False)
            if changed:
                dist._len = self._lenFunc(super()._getEvalLen())
                # Recompute metaData
                prevMtdt = dist._metaData
                dist._metaData = None
                dist.metaData
                # Compute correct _distrInd
                dist._distrInds = self._distribIndFunc(dist._distrInds)
                # Update labels
                dist._labels = [dist._labels[i] for i in dist._distrInds]
                # Add previously defined metadata, if applicable
                for key, vals in prevMtdt.items():
                    if key not in dist.metaData:
                        dist.metaData.__setitem__(key, [vals[i] for i in dist._distrInds], _internal=True)
            return dist, changed
        else:
            self._fullLen = self._getEvalLen()
            if not nsim.MPI._shouldWrite:
...
class _MemoryDataAccessor:
    ...
    def __array__(self, dtype=None, copy=None):
        res = numpy.asarray(nutils.nparray(self._data), dtype=dtype)
        return numpy.copy(res) if copy else res
...
class _FileDataAccessor(nutils.Versioned):
    ...
    def __array__(self, dtype=None, copy=None):
        res = numpy.asarray(self.__getitem__(slice(None, None, None), forceArray=True), dtype=dtype)
        return numpy.copy(res) if copy else res
    ...
    def __getitem__(self, key, forceArray=False):
        ...
        for rnk, (rs, resInds, locInds) in rnk2RsAndInds.items():
            ...
            if res is None:
                ...
                res = numpy.zeros((nbRuns, nbTpts, len(allColInds)), dtype=locData.dtype)
            res[:, :, resInds] = locData
...
class DatabaseHandler:
    ...
    # Custom exceptions
    class NoRunGroupsError(Exception):
        pass

    class TooManyRunGroupsError(Exception):
        pass
    ...
    def _initializeParameters(self):
        ...
        if self._parameters is None or self._param2Groups is None:
            ...
            self._param2Groups = {}
            for group in self:
                self._param2Groups.setdefault(None, set()).add(group)
    ...
    def filter(self, **kwargs):
        ...
        for keyVal in kwargs.items():
            ...
            if keyVal in self._param2Groups:
                ...
                res = set()
                break
        return res if res is not None else self._param2Groups[None]
    ...
    def get(self, **kwargs):
        ...
        if len(groups) == 0:
            raise DatabaseHandler.NoRunGroupsError(
                f'No run groups matched ({paramVals}).'
            )
        if len(groups) > 1:
            raise DatabaseHandler.TooManyRunGroupsError(
...
class HDF5Handler(DatabaseHandler, nutils.Versioned):
    ...
    :param pathPrefix: Path and prefix for the HDF5 file(s) (e.g. './data/HDF5Data' would
        ...
        is distributed.
    :type pathPrefix: str
    :param mode: Write mode for opening the :py:func:`h5py.File`. It is only used when writing data.
        Allowed values are `'w'` (create file, truncate if exists), `'x'` (create file, fail if exists),
        or `'a'` (append if exists, create otherwise). Defaults to `'a'`.
    :type mode: str
    ...
    Usage when saving::
        ...
        rs1.description = 'my_data'                       # Give a custom description to the
        ...                                               # result selector
    ...
    Usage when accessing data from the database::
        ...
        with HDF5Handler('./path/to/Prefix') as hdf:    # Create database handler.
            ...
            group = hdf['MySimulation']                 # Get the run group named 'MySimulation'

            val1 = group.val1                           # Querying a parameter value from
                                                        # the run group

            rs1, rs2, rs3 = group.results               # Querying all result selectors that
                                                        # were saved for the run group. They
                                                        # are returned in the same order as
                                                        # they were added to the simulation.
            # Or...
            rs1 = group.results['my_data']              # Accessing a single result selector by
                                                        # its description
    ...
    def __init__(self, pathPrefix, mode='a', hdf5FileKwArgs={}, hdf5DatasetKwArgs={}, internalKwArgs={}, **kwargs):
        super().__init__(**kwargs)
    ...
    def _close(self):
        ...
        for rnk, dbh in self._distribRankDBHs.items():
            if rnk != nsim.MPI._rank:
                dbh._close()
        self._distribRankDBHs = {}
        self._compObjHandler = None

    def _checkOpenFile(self, sim=None):
        if self._exited:
            raise Exception(
                'Cannot read or write from/to the HDF5 file, already exited the context manager'
            )
        ...
        if self._file is None:
            ...
            if sim is None:
                ...
                self._file = h5py.File(self._path, self._mode, **self._fileKwArgs)
    ...
    def __iter__(self):
        ...
        for name, group in self._file.items():
            if all(attr in group.attrs for attr in HDF5Handler._GROUP_DEFAULT_ATTRS):
                yield HDF5Group(self, group, version=group.attrs[HDF5Handler._STEPS_VERSION_ATTR_NAME])
...
class HDF5GroupResults(nutils.Versioned, nutils.ReadOnlyDictInterface):
    """A class containing the result selectors associated with a run group

    Can be used as a list by accessing result selectors by index, or as a
    dictionary by accessing result selectors by their description string.
    """

    def __init__(self, hdfgroup, **kwargs):
        super().__init__(**kwargs)
        ...
        distrGroupNames = [
            gn for gn in hdfgroup._group if re.match(HDF5Handler._RS_DISTGROUP_NAME.format(r'\d+'), gn) is not None
        ]
        if len(distrGroupNames) > 0:
            self._selectors = [
                _ReadOnlyResultSelector(
                    _HDF5DistribDataHandler(hdfgroup, hdfgroup._dbh, None, hdfgroup._group[gn], version=self._version)
                ) for gn in distrGroupNames
            ]
        else:
            self._selectors = [
                _ReadOnlyResultSelector(
                    _HDF5DataHandler(hdfgroup._dbh, None, hdfgroup._group[gn], version=self._version)
                ) for gn in hdfgroup._group if re.match(HDF5Handler._RS_GROUP_NAME.format(r'\d+'), gn) is not None
            ]

        self._fillDescriptionMap()

    @nutils.Versioned._versionRange(belowOrEq=_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE)
    def _fillDescriptionMap(self):
        # No description before 5.0.1, so skip that part in case we load an old file
        pass

    @nutils.Versioned._versionRange(above=_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE)
    def _fillDescriptionMap(self):
        for sel in self._selectors:
            self._descr2sel.setdefault(sel.description, sel)
    ...
    def __getitem__(self, key):
        """Access result selector through index or description

        :param key: The result selector index or the result selector
            description. The index depends on the order in which result selectors
            were added to the simulation with the
            :py:func:`steps.API_2.sim.Simulation.toSave` method. The description is
            automatically generated but can also be modified using
            :py:attr:`ResultSelector.description`.
        :type key: Union[int, str]

        :rtype: :py:class:`ResultSelector`, read-only

        .. note::
            If several result selectors share the same description, the result selector
            returned will be the one that was added first to the simulation with the
            :py:func:`steps.API_2.sim.Simulation.toSave` method.

        :meta public:
        """
        if isinstance(key, str):
            return self._getFromDescription(key)
        elif isinstance(key, numbers.Integral):
            return self._getFromIndex(key)
        else:
            raise TypeError(
                f'Expected a result selector index (int) or description(str), got {key} instead.'
            )

    @nutils.Versioned._versionRange(belowOrEq=_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE)
    def _getFromDescription(self, key):
        # Raise error by calling keys
        self.keys()

    @nutils.Versioned._versionRange(above=_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE)
    def _getFromDescription(self, key):
        try:
            return self._descr2sel[key]
        except KeyError:
            raise UnavailableSelectorError(key)

    def _getFromIndex(self, key):
        if 0 <= key < len(self._selectors):
            return self._selectors[key]
        else:
            raise IndexError(
                f'Result selector index should be >= 0 and < {len(self._selectors)}, got {key} instead.'
            )

    @nutils.Versioned._versionRange(belowOrEq=_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE)
    def keys(self):
        raise NotImplementedError(
            f'Result selectors loaded from files saved with STEPS < '
            f'{_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE} cannot be selected by description. '
            f'The current file was saved with STEPS {self._version}.'
        )

    @nutils.Versioned._versionRange(above=_DataHandler.DESCRIPTION_ADDED_VERSION_ABOVE)
    def keys(self):
        return self._descr2sel.keys()
...
class HDF5Group(DatabaseGroup, nutils.Versioned):
    ...
    def results(self):
        ...
        :type: :py:class:`HDF5GroupResults`, read-only

        See :py:class:`HDF5Handler` for usage examples.
        """
        return HDF5GroupResults(self, version=self._version)
...
class XDMFHandler(HDF5Handler):
    ...
    :type pathPrefix: str
    :param mode: see :py:class:`HDF5Handler`.
    :type mode: str
    ...
    def __init__(self, pathPrefix, mode='a', hdf5FileKwArgs={}, hdf5DatasetKwArgs={}, xdmfFolder=None, **kwargs):
        super().__init__(pathPrefix, mode, hdf5FileKwArgs, hdf5DatasetKwArgs, **kwargs)
    ...
    def _close(self):
        """Close the file"""
        if self._currGroup is not None:
            try:
                self._writeRuntimeInfo()
            except ValueError as ex:
                raise Exception(
                    f'Could not write runtime information to the HDF5 file because of the '
    ...
    def _newGroup(self, sim, uid, selectors, **kwargs):
        """Initialize the file and add a new run group."""
        # Write runtime info of previously opened group, if any
        if self._currGroup is not None:
            self._writeRuntimeInfo()
    ...
    def _writeRuntimeInfo(self):
        """Write a dictionary containing runtime information that can be useful for visualization"""
        # Only rank 0 writes this data
        if nsim.MPI._shouldWrite and self._sim is not None:
            sd = self._currGroup.staticData
            sd['RuntimeInfo'] = self._sim._runtimeInfo

...
class _SimPathDescr:
    ...
    def _getFunction(self, prefix, path):
        ...
        for i, e in enumerate(path):
            ...
            # TODO check that this also works for setting values
...
class SimPath:
    ...
    _PATH_END_UNITS = {
        ...
        'K': lambda path: path[-1]._getRateUnits(),
    ...
    def POINTSPECS(self, pointSpecs=None):
        ...
        be considered), or a :py:class:`steps.API_2.model.PointSpecies` (in which case only point species of
        the corresponding type will be considered).

        :param pointSpecs: The point species, a :py:class:`SimPath` that can be used to build a
            :py:class:`PointSpecList`, or a :py:class:`steps.API_2.model.PointSpecies`.
        :type pointSpecs: Union[:py:class:`PointSpecList`, :py:class:`SimPath`,
            :py:class:`steps.API_2.model.PointSpecies`]
        ...
        elif isinstance(pointSpecs, nmodel.PointSpecies):
            ...
            if len(subs) == 0:
                ...
                for name in names:
                    if name in e.children:
                        ...
                        not hasattr(e.__class__, '_SIMPATH_ONLY_CHILDREN')
...
class Simulation(nutils.NamedObject, nutils.StepsWrapperObject, nutils.AdvancedParameterizedObject):
    ...
    def __init__(self, solverName, mdl, geom, rng=None, *args, name=None, _createObj=True, **kwargs):
...
class _TypedSimObjectRerence(_SimObjectReference):
...
@nutils.FreezeAfterInit
class VesicleReference(_TypedSimObjectRerence):
    ...
    def setPos(self, pos, force=False):
        ...
        self._sim.solver.setCompSingleVesiclePos(self.Compartment, self._type, self._id, pos, force)
...
@nutils.FreezeAfterInit
class RaftReference(_TypedSimObjectRerence):
...
@nutils.FreezeAfterInit
class PointSpecReference(_TypedSimObjectRerence):
...
class _SimObjectList(nutils.SolverPathObject):
    ...
    def __getitem__(self, key):
        if self._specifArgs is not None:
            args, kwargs = self._specifArgs
            return self.__class__._refCls(self._sim, self._type, self._lst[key])(*args, **kwargs)
        else:
            return self.__class__._refCls(self._sim, self._type, self._lst[key])
...
class VesicleList(_SimObjectList):
    ...
    :param ves: Simulation path to the type of vesicle
    :type ves: :py:class:`SimPath`
    """
...
class RaftList(_SimObjectList):
    ...
    :param raft: Simulation path to the type of raft
    :type raft: :py:class:`SimPath`
    """
...
class PointSpecList(_SimObjectList):
    ...
    :param pointSpec: Simulation path to the type of point species
    :type pointSpec: :py:class:`SimPath`
    """
...
class LinkSpecList(_SimObjectList):
    ...
    :param linkSpec: Simulation path to the type of link species
    :type linkSpec: :py:class:`SimPath`
    """
...
class _ListStandIn(nutils.NamedObject, nutils.SolverRunTimeObject):
    ...
    """
    def __init__(self, listtpe, obj, **kwargs):

...
class ElementMap:
    ...
    This object contains the id mapping between elements in the imported file,
    and associated id in STEPS.

    Methods:
    insert(type, import_id, steps_id)
    Insert a mapping of element with import id (import_id), STEPS id (steps_id)
    and element type (type).

    getSTEPSID(type, import_id)
    Return a element's STEPS id from its type and import id.


    getImportID(type, steps_id)
    Return a element's import id from its type and STEPS id.
...
def importTetgen(pathroot):
    ...
    nodemap = ElementMap('node', 3);
    tetmap = ElementMap('tet', 4);
    trimap = ElementMap('tri', 3) 
    ...
    if (facefname != ''):
        ...
        while (trimap.getSize() != ntris):
            ...
            tri[1] = int(tokens[2])
            tri[2] = int(tokens[3])
            trimap.insert(triid, tri)
        # Close the file.
        facefile.close()

    print "Read TetGen files succesfully"    
    
    nodedata = nodemap.getAllData()
    tetdata = tetmap.getAllData() 
    tridata = trimap.getAllData() 

    print "creating Tetmesh object in STEPS..."
    mesh = stetmesh.Tetmesh(nodedata, tetdata, tridata)

    return (mesh, nodemap, tetmap, trimap)
...
def importAbaqus(filename, scale):
    ...
    the blocks information and the id mapping between the imported file and steps.
    ...
    Returns:
        1) mesh: The steps.gemo.TetMesh object storing the mesh data.
        2) blocks: a dictionary which stores the element block names,
           as well as the id boundaries (from, end) of each block.
        3) mapping: A ElementMap object storing the id mapping between the imported file
           and steps internal id system.
    ...
    while(line):
        ...
        if (line.find('**', 0, 2) == 0):
            ...
            lineset = line.split('=') 
            if (len(lineset) == 1):
                nodemap.blockBegin('AllNodes')
            else:
                nodemap.blockBegin(lineset[1].strip('\n'))
            currmap = nodemap
            ...
            if (elementtype == 'C3D4'):
                currmap = tetmap
                tetmap.blockBegin(blockname)
            elif (elementtype == 'STRI3'):
                currmap = trimap
                trimap.blockBegin(blockname)
            ...
            if (currmap == nodemap):
                ...
                node = [0,0,0,0]
                # set to the right scale
                node[0] = int(linesec[1])
                node[1] = int(linesec[2])
                node[2] = int(linesec[3]) 
                node[3] = int(linesec[4]) 
                currmap.insert(nodeid, node)
            elif (currmap == trimap):
                linesec = line.replace(',', ' ')
                linesec = linesec.split() 

...
(1) Abaqus data format (http://www.simulia.com)
    Abaqus format is supported by CUBIT (http://cubit.sandia.gov/)
    and Netgen (http://sourceforge.net/projects/netgen-mesher/)
(2) Tengen's data formats (http://tetgen.berlios.de/)
    Currently, only the .node, .ele and .face files are supported, 
    other Tetgen files is not used in STEPS.

Each import function (either importTetgen or importAbaqus) will read and import data from given
data file(s), returns a Tetmesh object for STEPS simulation, and three ElementProxy objects where
geometry data and id mapping are stored. Please type:

        help(steps.utilities.meshio.importAbaqus) or
        help(steps.utilities.meshio.importTetgen)

for the usage of importing function.

Type:
        help(steps.utilities.meshio.ElementProxy)

for avaliable methods of the ElementProxy object.
...
class ElementProxy:
    ...
    def getDataFromImportID(self, import_id):
        ...
        Parameters:
        - import_id (int): The import id of the element, this id is given by outside generator
...
""" Support for TetGen(http://tetgen.berlios.de/), 
a freely available tetrahedral mesh generator.
...
""" Supoprt for Abaqus format, which can be exported by CUBIT (http://cubit.sandia.gov/), 
and Netgen Mesh Generator (http://sourceforge.net/projects/netgen-mesher/)"""
...
def importAbaqus(filename, scale):
    ...
    PARAMETERS:
        1) filename: the Abaqus filename (or path) including any suffix.
        2) scale: LENGTH scale from the importing mesh to real geometry. e.g. a radius 
